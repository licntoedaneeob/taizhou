...
*/
	...
	private function CheckgameData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckgameData_Mod(&$User, $user_id=0){
...
/*互动游戏图集*/
    ...
    public function listing_gamephoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $this->success('删除成功', U('game/listing_gamephoto'));
            exit;
        }
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('gamephoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('互动游戏图集列表');
        $PageMenu = array(
            array( U('game/create_gamephoto'), L('添加互动游戏图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_gamephoto()
    {
        $module = $UserMod = M('gamephoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_gamephoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加互动游戏图集');
            $PageMenu = array(
                array( U('game/listing_gamephoto'), L('互动游戏图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_gamephoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($gameMod->create()) {
                ...
                $result =   $gameMod->save();
                if($result) {
                    $this->success('操作成功！', U('game/edit_gamephoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
