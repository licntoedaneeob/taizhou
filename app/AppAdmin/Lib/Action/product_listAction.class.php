 ...
 *
 */
class product_listAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_listMod = M('product_list');
            $sql=" id in (".$in.") ";
            $product_listMod->where($sql)->delete();

            $this->success('删除成功', U('product_list/listing'));
            exit;
        }
        ...
        $this->ModManager = M('product_list');
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('product_list', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('产品列表');
		$PageMenu = array(
			array( U('product_list/create'), L('添加产品') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkproduct_listData_Post();

            if (false === $product_listMod->create()) {
                $this->error($module->getError());
            }

            if($product_listMod->create()) {

                //$rst=$this->Checkproduct_listData_Mod($product_listMod);
                $product_listMod->create_time=time();
                $product_listMod->modify_time=time();

                $result =   $product_listMod->add();
                if($result) {
                    $this->success('操作成功！', U('product_list/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($product_listMod->getError());
            }
            ...
            $PageTitle = L('添加产品');
            $PageMenu = array(
                array( U('product_list/listing'), L('产品列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkproduct_listData_Post($id);

	        $product_listMod = M('product_list');

	        if($product_listMod->create()) {
	        	
                //$rst=$this->Checkproduct_listData_Mod($product_listMod,$id);
                $product_listMod->modify_time=time();

	            $result =   $product_listMod->save();
	            if($result) {
	                $this->success('操作成功！', U('product_list/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function Checkproduct_listData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkproduct_listData_Mod(&$User, $user_id=0){
...
/*产品图集*/
    ...
    public function listing_product_listphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_listMod = M('product_listphoto');
            $sql=" id in (".$in.") ";
            $product_listMod->where($sql)->delete();

            $this->success('删除成功', U('product_list/listing_product_listphoto'));
            exit;
        }
        ...
        $this->ModManager = M('product_listphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('product_listphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('产品图集列表');
        $PageMenu = array(
            array( U('product_list/create_product_listphoto'), L('添加产品图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_product_listphoto()
    {
        $module = $UserMod = M('product_listphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_product_listphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加产品图集');
            $PageMenu = array(
                array( U('product_list/listing_product_listphoto'), L('产品图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_product_listphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($product_listMod->create()) {
                ...
                $result =   $product_listMod->save();
                if($result) {
                    $this->success('操作成功！', U('product_list/edit_product_listphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*产品设计*/
    ...
    public function listing_product_listdesign()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_listMod = M('product_listdesign');
            $sql=" id in (".$in.") ";
            $product_listMod->where($sql)->delete();

            $this->success('删除成功', U('product_list/listing_product_listdesign'));
            exit;
        }
        ...
        $this->ModManager = M('product_listdesign');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('product_listdesign', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('产品图集列表');
        $PageMenu = array(
            array( U('product_list/create_product_listdesign'), L('添加产品图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_product_listdesign()
    {
        $module = $UserMod = M('product_listdesign');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_product_listdesign()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加产品图集');
            $PageMenu = array(
                array( U('product_list/listing_product_listdesign'), L('产品图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_product_listdesign()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($product_listMod->create()) {
                ...
                $result =   $product_listMod->save();
                if($result) {
                    $this->success('操作成功！', U('product_list/edit_product_listdesign', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
