 ...
 *
 */
class competitionztAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $competitionztMod = M('competitionzt');
            $sql=" id in (".$in.") ";
            $competitionztMod->where($sql)->delete();

            $this->success('删除成功', U('competitionzt/listing'));
            exit;
        }
        ...
        $this->ModManager = M('competitionzt');
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('competitionzt', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('红牛专题赛事列表');
		$PageMenu = array(
			array( U('competitionzt/create'), L('添加红牛专题赛事') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckcompetitionztData_Post();

            if (false === $competitionztMod->create()) {
                $this->error($module->getError());
            }

            if($competitionztMod->create()) {

                //$rst=$this->CheckcompetitionztData_Mod($competitionztMod);
                $competitionztMod->create_time=time();
                $competitionztMod->modify_time=time();

                $result =   $competitionztMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
                ...
                $this->error($competitionztMod->getError());
            }

        }else{

            $PageTitle = L('添加红牛专题赛事');
            $PageMenu = array(
                array( U('competitionzt/listing'), L('红牛专题赛事列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckcompetitionztData_Post($id);

	        $competitionztMod = M('competitionzt');

	        if($competitionztMod->create()) {
	        	
                //$rst=$this->CheckcompetitionztData_Mod($competitionztMod,$id);
                $competitionztMod->modify_time=time();

	            $result =   $competitionztMod->save();
	            if($result) {
	                $this->success('操作成功！', U('competitionzt/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
	...
	{
		$module = $UserMod = M('competitionzt');
...
*/
	...
	private function CheckcompetitionztData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckcompetitionztData_Mod(&$User, $user_id=0){
...
/*红牛专题赛事图集*/
    ...
    public function listing_competitionztphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $competitionztMod = M('competitionztphoto');
            $sql=" id in (".$in.") ";
            $competitionztMod->where($sql)->delete();

            $this->success('删除成功', U('competitionzt/listing_competitionztphoto'));
            exit;
        }
        ...
        $this->ModManager = M('competitionztphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('competitionztphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('红牛专题赛事图集列表');
        $PageMenu = array(
            array( U('competitionzt/create_competitionztphoto'), L('添加红牛专题赛事图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_competitionztphoto()
    {
        $module = $UserMod = M('competitionztphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_competitionztphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加红牛专题赛事图集');
            $PageMenu = array(
                array( U('competitionzt/listing_competitionztphoto'), L('红牛专题赛事图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_competitionztphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($competitionztMod->create()) {
                ...
                $result =   $competitionztMod->save();
                if($result) {
                    $this->success('操作成功！', U('competitionzt/edit_competitionztphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
