 ...
 *
 */
class starAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $starMod = M('star');
            $sql=" id in (".$in.") ";
            $starMod->where($sql)->delete();

            $this->success('删除成功', U('star/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('star', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('能量大咖列表');
		$PageMenu = array(
			array( U('star/create'), L('添加能量大咖') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckstarData_Post();

            if (false === $starMod->create()) {
                $this->error($module->getError());
            }

            if($starMod->create()) {

                //$rst=$this->CheckstarData_Mod($starMod);
                $starMod->create_time=time();
                $starMod->modify_time=time();
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckstarData_Post($id);

	        $starMod = M('star');

	        if($starMod->create()) {
	        	
                //$rst=$this->CheckstarData_Mod($starMod,$id);
                $starMod->modify_time=time();

	            $result =   $starMod->save();
	            if($result) {
	                $this->success('操作成功！', U('star/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckstarData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckstarData_Mod(&$User, $user_id=0){
...
/*能量大咖图集*/
    ...
    public function listing_starphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $this->success('删除成功', U('star/listing_starphoto'));
            exit;
        }
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('starphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('能量大咖图集列表');
        $PageMenu = array(
            array( U('star/create_starphoto'), L('添加能量大咖图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_starphoto()
    {
        $module = $UserMod = M('starphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_starphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加能量大咖图集');
            $PageMenu = array(
                array( U('star/listing_starphoto'), L('能量大咖图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_starphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($starMod->create()) {
                ...
                $result =   $starMod->save();
                if($result) {
                    $this->success('操作成功！', U('star/edit_starphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
