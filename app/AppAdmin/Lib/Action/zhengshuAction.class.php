 ...
 *
 */
class zhengshuAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $zhengshuMod = M('zhengshu');
            $sql=" id in (".$in.") ";
            $zhengshuMod->where($sql)->delete();

            $this->success('删除成功', U('zhengshu/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('zhengshu', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('证书列表');
		$PageMenu = array(
			array( U('zhengshu/create'), L('添加证书') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckzhengshuData_Post();

            if (false === $zhengshuMod->create()) {
                $this->error($module->getError());
            }

            if($zhengshuMod->create()) {

                //$rst=$this->CheckzhengshuData_Mod($zhengshuMod);
                $zhengshuMod->create_time=time();
                $zhengshuMod->modify_time=time();

                $result =   $zhengshuMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
                ...
                $this->error($zhengshuMod->getError());
            }
            ...
            $PageTitle = L('添加证书');
            $PageMenu = array(
                array( U('zhengshu/listing'), L('证书列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckzhengshuData_Post($id);

	        $zhengshuMod = M('zhengshu');

	        if($zhengshuMod->create()) {
	        	
                //$rst=$this->CheckzhengshuData_Mod($zhengshuMod,$id);
                $zhengshuMod->modify_time=time();

	            $result =   $zhengshuMod->save();
	            if($result) {
	                $this->success('操作成功！', U('zhengshu/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckzhengshuData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckzhengshuData_Mod(&$User, $user_id=0){
...
/*证书图集*/
    ...
    public function listing_zhengshuphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $zhengshuMod = M('zhengshuphoto');
            $sql=" id in (".$in.") ";
            $zhengshuMod->where($sql)->delete();

            $this->success('删除成功', U('zhengshu/listing_zhengshuphoto'));
            exit;
        }
        ...
        $this->ModManager = M('zhengshuphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('zhengshuphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('证书图集列表');
        $PageMenu = array(
            array( U('zhengshu/create_zhengshuphoto'), L('添加证书图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_zhengshuphoto()
    {
        $module = $UserMod = M('zhengshuphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_zhengshuphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加证书图集');
            $PageMenu = array(
                array( U('zhengshu/listing_zhengshuphoto'), L('证书图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_zhengshuphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($zhengshuMod->create()) {
                ...
                $result =   $zhengshuMod->save();
                if($result) {
                    $this->success('操作成功！', U('zhengshu/edit_zhengshuphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
