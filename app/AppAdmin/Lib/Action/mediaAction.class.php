 ...
 *
 */
class mediaAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $mediaMod = M('media');
            $sql=" id in (".$in.") ";
            $mediaMod->where($sql)->delete();

            $this->success('删除成功', U('media/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('media', $sqlWhere, $sqlOrder, '', 'M');
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckmediaData_Post();

            if (false === $mediaMod->create()) {
                $this->error($module->getError());
            }

            if($mediaMod->create()) {

                //$rst=$this->CheckmediaData_Mod($mediaMod);
                $mediaMod->create_time=time();
                $mediaMod->modify_time=time();

                $result =   $mediaMod->add();
                if($result) {
                    //$this->success('操作成功！', U('media/create'));
                    $this->success('操作成功！', U('media/listing'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($mediaMod->getError());
            }
            ...
            $PageTitle = L('添加合作伙伴');
            $PageMenu = array(
                array( U('media/listing'), L('合作伙伴列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckmediaData_Post($id);

	        $mediaMod = M('media');

	        if($mediaMod->create()) {
	        	
                //$rst=$this->CheckmediaData_Mod($mediaMod,$id);
                $mediaMod->modify_time=time();

	            $result =   $mediaMod->save();
	            if($result) {
	                //$this->success('操作成功！', U('media/edit', array('id'=>$id)) );
	                $this->success('操作成功！', U('media/listing'));
	            }else{
	                $this->error('写入错误！');
	            }
...
*/
	...
	private function CheckmediaData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckmediaData_Mod(&$User, $user_id=0){
...
/*合作伙伴图集*/
    ...
    public function listing_mediaphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $mediaMod = M('mediaphoto');
            $sql=" id in (".$in.") ";
            $mediaMod->where($sql)->delete();

            $this->success('删除成功', U('media/listing_mediaphoto'));
            exit;
        }
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('mediaphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageMenu = array(
            array( U('media/create_mediaphoto'), L('添加合作伙伴图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_mediaphoto()
    {
        $module = $UserMod = M('mediaphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_mediaphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageMenu = array(
                array( U('media/listing_mediaphoto'), L('合作伙伴图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_mediaphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($mediaMod->create()) {
                ...
                $result =   $mediaMod->save();
                if($result) {
                    $this->success('操作成功！', U('media/edit_mediaphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*合作伙伴行程*/
    ...
    public function listing_mediacircle()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $mediaMod = M('mediacircle');
            $sql=" id in (".$in.") ";
            $mediaMod->where($sql)->delete();

            $this->success('删除成功', U('media/listing_mediacircle'));
            exit;
        }
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('mediacircle', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('合作伙伴行程列表');
        $PageMenu = array(
            array( U('media/create_mediacircle'), L('添加合作伙伴行程') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_mediacircle()
    {
        $module = $UserMod = M('mediacircle');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_mediacircle()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加合作伙伴行程');
            $PageMenu = array(
                array( U('media/listing_mediacircle'), L('合作伙伴行程列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_mediacircle()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($mediaMod->create()) {
                ...
                $result =   $mediaMod->save();
                if($result) {
                    $this->success('操作成功！', U('media/edit_mediacircle', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
