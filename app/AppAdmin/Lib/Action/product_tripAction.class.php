 ...
 *
 */
class product_tripAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_tripMod = M('product_trip');
            $sql=" id in (".$in.") ";
            $product_tripMod->where($sql)->delete();

            $this->success('删除成功', U('product_trip/listing'));
            exit;
        }
        ...
        $this->ModManager = M('product_trip');
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('product_trip', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('路线行程安排列表');
		$PageMenu = array(
			array( U('product_trip/create'), L('添加路线行程安排') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkproduct_tripData_Post();

            if (false === $product_tripMod->create()) {
                $this->error($module->getError());
            }

            if($product_tripMod->create()) {

                //$rst=$this->Checkproduct_tripData_Mod($product_tripMod);
                $product_tripMod->create_time=time();
                $product_tripMod->modify_time=time();

                $result =   $product_tripMod->add();
                if($result) {
                    $this->success('操作成功！', U('product_trip/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($product_tripMod->getError());
            }

        }else{

            $PageTitle = L('添加路线行程安排');
            $PageMenu = array(
                array( U('product_trip/listing'), L('路线行程安排列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkproduct_tripData_Post($id);

	        $product_tripMod = M('product_trip');

	        if($product_tripMod->create()) {
	        	
                //$rst=$this->Checkproduct_tripData_Mod($product_tripMod,$id);
                $product_tripMod->modify_time=time();

	            $result =   $product_tripMod->save();
	            if($result) {
	                $this->success('操作成功！', U('product_trip/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function Checkproduct_tripData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkproduct_tripData_Mod(&$User, $user_id=0){
...
/*路线行程安排图集*/
    ...
    public function listing_product_tripphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_tripMod = M('product_tripphoto');
            $sql=" id in (".$in.") ";
            $product_tripMod->where($sql)->delete();

            $this->success('删除成功', U('product_trip/listing_product_tripphoto'));
            exit;
        }
        ...
        $this->ModManager = M('product_tripphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('product_tripphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('路线行程安排图集列表');
        $PageMenu = array(
            array( U('product_trip/create_product_tripphoto'), L('添加路线行程安排图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_product_tripphoto()
    {
        $module = $UserMod = M('product_tripphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_product_tripphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加路线行程安排图集');
            $PageMenu = array(
                array( U('product_trip/listing_product_tripphoto'), L('路线行程安排图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_product_tripphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($product_tripMod->create()) {
                ...
                $result =   $product_tripMod->save();
                if($result) {
                    $this->success('操作成功！', U('product_trip/edit_product_tripphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*路线行程安排行程*/
    ...
    public function listing_product_tripcircle()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_tripMod = M('product_tripcircle');
            $sql=" id in (".$in.") ";
            $product_tripMod->where($sql)->delete();

            $this->success('删除成功', U('product_trip/listing_product_tripcircle'));
            exit;
        }
        ...
        $this->ModManager = M('product_tripcircle');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('product_tripcircle', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('路线行程安排行程列表');
        $PageMenu = array(
            array( U('product_trip/create_product_tripcircle'), L('添加路线行程安排行程') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_product_tripcircle()
    {
        $module = $UserMod = M('product_tripcircle');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_product_tripcircle()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加路线行程安排行程');
            $PageMenu = array(
                array( U('product_trip/listing_product_tripcircle'), L('路线行程安排行程列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_product_tripcircle()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($product_tripMod->create()) {
                ...
                $result =   $product_tripMod->save();
                if($result) {
                    $this->success('操作成功！', U('product_trip/edit_product_tripcircle', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
