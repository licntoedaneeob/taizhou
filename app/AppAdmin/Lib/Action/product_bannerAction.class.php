 ...
 *
 */
class product_bannerAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_bannerMod = M('product_banner');
            $sql=" id in (".$in.") ";
            $product_bannerMod->where($sql)->delete();

            $this->success('删除成功', U('product_banner/listing'));
            exit;
        }
        ...
        $this->ModManager = M('product_banner');
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('product_banner', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageMenu = array(
			array( U('product_banner/create'), L('添加路线Banner图') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkproduct_bannerData_Post();

            if (false === $product_bannerMod->create()) {
                $this->error($module->getError());
            }

            if($product_bannerMod->create()) {

                //$rst=$this->Checkproduct_bannerData_Mod($product_bannerMod);
                $product_bannerMod->create_time=time();
                $product_bannerMod->modify_time=time();

                $result =   $product_bannerMod->add();
                if($result) {
                    $this->success('操作成功！', U('product_banner/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($product_bannerMod->getError());
            }
            ...
            $PageMenu = array(
                array( U('product_banner/listing'), L('路线Banner图列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkproduct_bannerData_Post($id);

	        $product_bannerMod = M('product_banner');

	        if($product_bannerMod->create()) {
	        	
                //$rst=$this->Checkproduct_bannerData_Mod($product_bannerMod,$id);
                $product_bannerMod->modify_time=time();

	            $result =   $product_bannerMod->save();
	            if($result) {
	                $this->success('操作成功！', U('product_banner/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
	        ...
	        $product_bannerMod = M('product_banner');
		    // 读取数据
		    $data =   $product_bannerMod->find($id);
		    if($data) {
	...
	{
		$module = $UserMod = M('product_banner');
...
*/
	...
	private function Checkproduct_bannerData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkproduct_bannerData_Mod(&$User, $user_id=0){
...
/*路线Banner图图集*/
    ...
    public function listing_product_bannerphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_bannerMod = M('product_bannerphoto');
            $sql=" id in (".$in.") ";
            $product_bannerMod->where($sql)->delete();

            $this->success('删除成功', U('product_banner/listing_product_bannerphoto'));
            exit;
        }
        ...
        $this->ModManager = M('product_bannerphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('product_bannerphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageMenu = array(
            array( U('product_banner/create_product_bannerphoto'), L('添加路线Banner图图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_product_bannerphoto()
    {
        $module = $UserMod = M('product_bannerphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_product_bannerphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageMenu = array(
                array( U('product_banner/listing_product_bannerphoto'), L('路线Banner图图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_product_bannerphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($product_bannerMod->create()) {
                ...
                $result =   $product_bannerMod->save();
                if($result) {
                    $this->success('操作成功！', U('product_banner/edit_product_bannerphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*路线Banner图行程*/
    ...
    public function listing_product_bannercircle()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_bannerMod = M('product_bannercircle');
            $sql=" id in (".$in.") ";
            $product_bannerMod->where($sql)->delete();

            $this->success('删除成功', U('product_banner/listing_product_bannercircle'));
            exit;
        }
        ...
        $this->ModManager = M('product_bannercircle');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('product_bannercircle', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageMenu = array(
            array( U('product_banner/create_product_bannercircle'), L('添加路线Banner图行程') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_product_bannercircle()
    {
        $module = $UserMod = M('product_bannercircle');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_product_bannercircle()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageMenu = array(
                array( U('product_banner/listing_product_bannercircle'), L('路线Banner图行程列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_product_bannercircle()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($product_bannerMod->create()) {
                ...
                $result =   $product_bannerMod->save();
                if($result) {
                    $this->success('操作成功！', U('product_banner/edit_product_bannercircle', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
