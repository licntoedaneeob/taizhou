 ...
 *
 */
class product_list_hotelAction extends TAction
{
...
//    }
	...
	{	
        ...
        $hotellist=$this->getAllHotelList();
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_list_hotelMod = M('product_list_hotel');
            $sql=" id in (".$in.") ";
            $product_list_hotelMod->where($sql)->delete();

            $this->success('删除成功', U('product_list_hotel/listing'));
            exit;
        }
        ...
        $filter_hotel_id = $this->REQUEST('_filter_hotel_id');
        
        if( $filter_hotel_id != '' ){
            $sqlWhere .= " and hotel_id = ". addslashes($filter_hotel_id)." ";
        }
        ...
        $this->ModManager = M('product_list_hotel');
        ...
        $this->assign('filter_hotel_id',  $filter_hotel_id);
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('product_list_hotel', $sqlWhere, $sqlOrder, '', 'M');
        ...
        if(isset($rst['dataset'])){
            foreach($rst['dataset'] as $k => $v){
                ...
                if(isset($hotellist[$v['hotel_id']])){
                    $rst['dataset'][$k]['hotel_name']=$hotellist[$v['hotel_id']]['title'];
                }
                else{
                    $rst['dataset'][$k]['hotel_name']="";
                }
		...
		$PageTitle = L('路线酒店列表');
		$PageMenu = array(
			array( U('product_list_hotel/create'), L('添加路线酒店') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkproduct_list_hotelData_Post();

            if (false === $product_list_hotelMod->create()) {
                $this->error($module->getError());
            }

            if($product_list_hotelMod->create()) {

                //$rst=$this->Checkproduct_list_hotelData_Mod($product_list_hotelMod);
                $product_list_hotelMod->create_time=time();
                $product_list_hotelMod->modify_time=time();

                $result =   $product_list_hotelMod->add();
                if($result) {
                    $this->success('操作成功！', U('product_list_hotel/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($product_list_hotelMod->getError());
            }
            ...
            $PageTitle = L('添加路线酒店');
            $PageMenu = array(
                array( U('product_list_hotel/listing'), L('路线酒店列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkproduct_list_hotelData_Post($id);

	        $product_list_hotelMod = M('product_list_hotel');

	        if($product_list_hotelMod->create()) {
	        	
                //$rst=$this->Checkproduct_list_hotelData_Mod($product_list_hotelMod,$id);
                $product_list_hotelMod->modify_time=time();

	            $result =   $product_list_hotelMod->save();
	            if($result) {
	                $this->success('操作成功！', U('product_list_hotel/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
	        ...
	        $product_list_hotelMod = M('product_list_hotel');
		    // 读取数据
		    $data =   $product_list_hotelMod->find($id);
		    if($data) {
	...
	{
		$module = $UserMod = M('product_list_hotel');
...
*/
	...
	private function Checkproduct_list_hotelData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkproduct_list_hotelData_Mod(&$User, $user_id=0){
...
/*路线酒店图集*/
    ...
    public function listing_product_list_hotelphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_list_hotelMod = M('product_list_hotelphoto');
            $sql=" id in (".$in.") ";
            $product_list_hotelMod->where($sql)->delete();

            $this->success('删除成功', U('product_list_hotel/listing_product_list_hotelphoto'));
            exit;
        }
        ...
        $this->ModManager = M('product_list_hotelphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('product_list_hotelphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('路线酒店图集列表');
        $PageMenu = array(
            array( U('product_list_hotel/create_product_list_hotelphoto'), L('添加路线酒店图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_product_list_hotelphoto()
    {
        $module = $UserMod = M('product_list_hotelphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_product_list_hotelphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加路线酒店图集');
            $PageMenu = array(
                array( U('product_list_hotel/listing_product_list_hotelphoto'), L('路线酒店图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_product_list_hotelphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($product_list_hotelMod->create()) {
                ...
                $result =   $product_list_hotelMod->save();
                if($result) {
                    $this->success('操作成功！', U('product_list_hotel/edit_product_list_hotelphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*路线酒店行程*/
    ...
    public function listing_product_list_hotelcircle()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_list_hotelMod = M('product_list_hotelcircle');
            $sql=" id in (".$in.") ";
            $product_list_hotelMod->where($sql)->delete();

            $this->success('删除成功', U('product_list_hotel/listing_product_list_hotelcircle'));
            exit;
        }
        ...
        $this->ModManager = M('product_list_hotelcircle');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('product_list_hotelcircle', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('路线酒店行程列表');
        $PageMenu = array(
            array( U('product_list_hotel/create_product_list_hotelcircle'), L('添加路线酒店行程') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_product_list_hotelcircle()
    {
        $module = $UserMod = M('product_list_hotelcircle');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_product_list_hotelcircle()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加路线酒店行程');
            $PageMenu = array(
                array( U('product_list_hotel/listing_product_list_hotelcircle'), L('路线酒店行程列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_product_list_hotelcircle()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($product_list_hotelMod->create()) {
                ...
                $result =   $product_list_hotelMod->save();
                if($result) {
                    $this->success('操作成功！', U('product_list_hotel/edit_product_list_hotelcircle', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
