 ...
 *
 */
class course_classAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $course_classMod = M('course_class');
            $sql=" id in (".$in.") ";
            $course_classMod->where($sql)->delete();

            $this->success('删除成功', U('course_class/listing'));
            exit;
        }
        ...
        $this->ModManager = M('course_class');
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('course_class', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('课程分类列表');
		$PageMenu = array(
			array( U('course_class/create'), L('添加课程分类') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkcourse_classData_Post();

            if (false === $course_classMod->create()) {
                $this->error($module->getError());
            }

            if($course_classMod->create()) {

                //$rst=$this->Checkcourse_classData_Mod($course_classMod);
                $course_classMod->create_time=time();
                $course_classMod->modify_time=time();

                $result =   $course_classMod->add();
                if($result) {
                    $this->success('操作成功！', U('course_class/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($course_classMod->getError());
            }
            ...
            $PageTitle = L('添加课程分类');
            $PageMenu = array(
                array( U('course_class/listing'), L('课程分类列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkcourse_classData_Post($id);

	        $course_classMod = M('course_class');

	        if($course_classMod->create()) {
	        	
                //$rst=$this->Checkcourse_classData_Mod($course_classMod,$id);
                $course_classMod->modify_time=time();

	            $result =   $course_classMod->save();
	            if($result) {
	                $this->success('操作成功！', U('course_class/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function Checkcourse_classData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkcourse_classData_Mod(&$User, $user_id=0){
...
/*课程分类图集*/
    ...
    public function listing_course_classphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $course_classMod = M('course_classphoto');
            $sql=" id in (".$in.") ";
            $course_classMod->where($sql)->delete();

            $this->success('删除成功', U('course_class/listing_course_classphoto'));
            exit;
        }
        ...
        $this->ModManager = M('course_classphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('course_classphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('课程分类图集列表');
        $PageMenu = array(
            array( U('course_class/create_course_classphoto'), L('添加课程分类图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_course_classphoto()
    {
        $module = $UserMod = M('course_classphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_course_classphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加课程分类图集');
            $PageMenu = array(
                array( U('course_class/listing_course_classphoto'), L('课程分类图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_course_classphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($course_classMod->create()) {
                ...
                $result =   $course_classMod->save();
                if($result) {
                    $this->success('操作成功！', U('course_class/edit_course_classphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*课程分类设计*/
    ...
    public function listing_course_classdesign()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $course_classMod = M('course_classdesign');
            $sql=" id in (".$in.") ";
            $course_classMod->where($sql)->delete();

            $this->success('删除成功', U('course_class/listing_course_classdesign'));
            exit;
        }
        ...
        $this->ModManager = M('course_classdesign');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('course_classdesign', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('课程分类图集列表');
        $PageMenu = array(
            array( U('course_class/create_course_classdesign'), L('添加课程分类图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_course_classdesign()
    {
        $module = $UserMod = M('course_classdesign');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_course_classdesign()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加课程分类图集');
            $PageMenu = array(
                array( U('course_class/listing_course_classdesign'), L('课程分类图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_course_classdesign()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($course_classMod->create()) {
                ...
                $result =   $course_classMod->save();
                if($result) {
                    $this->success('操作成功！', U('course_class/edit_course_classdesign', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
