 ...
 *
 */
class downloadAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $downloadMod = M('download');
            $sql=" id in (".$in.") ";
            $downloadMod->where($sql)->delete();

            $this->success('删除成功', U('download/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('download', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('炫酷下载列表');
		$PageMenu = array(
			array( U('download/create'), L('添加炫酷下载') ),
		);
    ...
    public function create()
    {
      ...
      $allclasslist=$this->getAllClassList();
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckdownloadData_Post();

            if (false === $downloadMod->create()) {
                $this->error($module->getError());
            }

            if($downloadMod->create()) {

                //$rst=$this->CheckdownloadData_Mod($downloadMod);
                $downloadMod->create_time=time();
                $downloadMod->modify_time=time();

                $result =   $downloadMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
                ...
                $this->error($downloadMod->getError());
            }
            ...
            $PageTitle = L('添加炫酷下载');
            $PageMenu = array(
                array( U('download/listing'), L('炫酷下载列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
      ...
      $allclasslist=$this->getAllClassList();
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckdownloadData_Post($id);

	        $downloadMod = M('download');

	        if($downloadMod->create()) {
	        	
                //$rst=$this->CheckdownloadData_Mod($downloadMod,$id);
                $downloadMod->modify_time=time();

	            $result =   $downloadMod->save();
	            if($result) {
	                $this->success('操作成功！', U('download/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckdownloadData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckdownloadData_Mod(&$User, $user_id=0){
...
/*炫酷下载图集*/
    ...
    public function listing_downloadphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $downloadMod = M('downloadphoto');
            $sql=" id in (".$in.") ";
            $downloadMod->where($sql)->delete();

            $this->success('删除成功', U('download/listing_downloadphoto'));
            exit;
        }
        ...
        $this->ModManager = M('downloadphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('downloadphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('炫酷下载图集列表');
        $PageMenu = array(
            array( U('download/create_downloadphoto'), L('添加炫酷下载图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_downloadphoto()
    {
        $module = $UserMod = M('downloadphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_downloadphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加炫酷下载图集');
            $PageMenu = array(
                array( U('download/listing_downloadphoto'), L('炫酷下载图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_downloadphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($downloadMod->create()) {
                ...
                $result =   $downloadMod->save();
                if($result) {
                    $this->success('操作成功！', U('download/edit_downloadphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
