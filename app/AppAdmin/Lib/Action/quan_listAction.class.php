 ...
 *
 */
class quan_listAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $quan_listMod = M('quan_list');
            $sql=" id in (".$in.") ";
            $quan_listMod->where($sql)->delete();

            $this->success('删除成功', U('quan_list/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('quan_list', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('卡券列表');
		$PageMenu = array(
			array( U('quan_list/create'), L('添加卡券') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkquan_listData_Post();

            if (false === $quan_listMod->create()) {
                $this->error($module->getError());
            }

            if($quan_listMod->create()) {

                //$rst=$this->Checkquan_listData_Mod($quan_listMod);
                $quan_listMod->create_time=time();
                $quan_listMod->modify_time=time();

                $result =   $quan_listMod->add();
                if($result) {
                    $this->success('操作成功！', U('quan_list/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($quan_listMod->getError());
            }
            ...
            $PageTitle = L('添加卡券');
            $PageMenu = array(
                array( U('quan_list/listing'), L('卡券列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkquan_listData_Post($id);

	        $quan_listMod = M('quan_list');

	        if($quan_listMod->create()) {
	        	
                //$rst=$this->Checkquan_listData_Mod($quan_listMod,$id);
                $quan_listMod->modify_time=time();

	            $result =   $quan_listMod->save();
	            if($result) {
	                $this->success('操作成功！', U('quan_list/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function Checkquan_listData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkquan_listData_Mod(&$User, $user_id=0){
...
/*卡券图集*/
    ...
    public function listing_quan_listphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $quan_listMod = M('quan_listphoto');
            $sql=" id in (".$in.") ";
            $quan_listMod->where($sql)->delete();

            $this->success('删除成功', U('quan_list/listing_quan_listphoto'));
            exit;
        }
        ...
        $this->ModManager = M('quan_listphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('quan_listphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('卡券图集列表');
        $PageMenu = array(
            array( U('quan_list/create_quan_listphoto'), L('添加卡券图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_quan_listphoto()
    {
        $module = $UserMod = M('quan_listphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_quan_listphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加卡券图集');
            $PageMenu = array(
                array( U('quan_list/listing_quan_listphoto'), L('卡券图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_quan_listphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($quan_listMod->create()) {
                ...
                $result =   $quan_listMod->save();
                if($result) {
                    $this->success('操作成功！', U('quan_list/edit_quan_listphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*卡券设计*/
    ...
    public function listing_quan_listdesign()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $quan_listMod = M('quan_listdesign');
            $sql=" id in (".$in.") ";
            $quan_listMod->where($sql)->delete();

            $this->success('删除成功', U('quan_list/listing_quan_listdesign'));
            exit;
        }
        ...
        $this->ModManager = M('quan_listdesign');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('quan_listdesign', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('卡券图集列表');
        $PageMenu = array(
            array( U('quan_list/create_quan_listdesign'), L('添加卡券图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_quan_listdesign()
    {
        $module = $UserMod = M('quan_listdesign');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_quan_listdesign()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加卡券图集');
            $PageMenu = array(
                array( U('quan_list/listing_quan_listdesign'), L('卡券图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_quan_listdesign()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($quan_listMod->create()) {
                ...
                $result =   $quan_listMod->save();
                if($result) {
                    $this->success('操作成功！', U('quan_list/edit_quan_listdesign', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
            ...
            do{
                ...
                if ($toShow['banner'][$k]['status_pay']=="2"){
                    ...
                    $output .= $toShow['banner'][$k]['id']
                    	...
                    	.$expstr.$toShow['banner'][$k]['quan_name']
                    	.$expstr.$toShow['banner'][$k]['quan_other']
