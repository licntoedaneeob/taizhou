 ...
 *
 */
class consultant_listAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $consultant_listMod = M('consultant_list');
            $sql=" id in (".$in.") ";
            $consultant_listMod->where($sql)->delete();

            $this->success('删除成功', U('consultant_list/listing'));
            exit;
        }
        ...
        $this->ModManager = M('consultant_list');
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('consultant_list', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('顾问列表');
		$PageMenu = array(
			array( U('consultant_list/create'), L('添加顾问') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkconsultant_listData_Post();

            if (false === $consultant_listMod->create()) {
                $this->error($module->getError());
            }

            if($consultant_listMod->create()) {

                //$rst=$this->Checkconsultant_listData_Mod($consultant_listMod);
                $consultant_listMod->create_time=time();
                $consultant_listMod->modify_time=time();

                $result =   $consultant_listMod->add();
                if($result) {
                    $this->success('操作成功！', U('consultant_list/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($consultant_listMod->getError());
            }
            ...
            $PageTitle = L('添加顾问');
            $PageMenu = array(
                array( U('consultant_list/listing'), L('顾问列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkconsultant_listData_Post($id);

	        $consultant_listMod = M('consultant_list');

	        if($consultant_listMod->create()) {
	        	
                //$rst=$this->Checkconsultant_listData_Mod($consultant_listMod,$id);
                $consultant_listMod->modify_time=time();

	            $result =   $consultant_listMod->save();
	            if($result) {
	                $this->success('操作成功！', U('consultant_list/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
	        ...
	        $consultant_listMod = M('consultant_list');
		    // 读取数据
		    $data =   $consultant_listMod->find($id);
		    if($data) {
	...
	{
		$module = $UserMod = M('consultant_list');
...
*/
	...
	private function Checkconsultant_listData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkconsultant_listData_Mod(&$User, $user_id=0){
...
/*顾问图集*/
    ...
    public function listing_consultant_listphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $consultant_listMod = M('consultant_listphoto');
            $sql=" id in (".$in.") ";
            $consultant_listMod->where($sql)->delete();

            $this->success('删除成功', U('consultant_list/listing_consultant_listphoto'));
            exit;
        }
        ...
        $this->ModManager = M('consultant_listphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('consultant_listphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('顾问图集列表');
        $PageMenu = array(
            array( U('consultant_list/create_consultant_listphoto'), L('添加顾问图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_consultant_listphoto()
    {
        $module = $UserMod = M('consultant_listphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_consultant_listphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加顾问图集');
            $PageMenu = array(
                array( U('consultant_list/listing_consultant_listphoto'), L('顾问图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_consultant_listphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($consultant_listMod->create()) {
                ...
                $result =   $consultant_listMod->save();
                if($result) {
                    $this->success('操作成功！', U('consultant_list/edit_consultant_listphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*顾问设计*/
    ...
    public function listing_consultant_listdesign()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $consultant_listMod = M('consultant_listdesign');
            $sql=" id in (".$in.") ";
            $consultant_listMod->where($sql)->delete();

            $this->success('删除成功', U('consultant_list/listing_consultant_listdesign'));
            exit;
        }
        ...
        $this->ModManager = M('consultant_listdesign');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('consultant_listdesign', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('顾问图集列表');
        $PageMenu = array(
            array( U('consultant_list/create_consultant_listdesign'), L('添加顾问图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_consultant_listdesign()
    {
        $module = $UserMod = M('consultant_listdesign');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_consultant_listdesign()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加顾问图集');
            $PageMenu = array(
                array( U('consultant_list/listing_consultant_listdesign'), L('顾问图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_consultant_listdesign()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($consultant_listMod->create()) {
                ...
                $result =   $consultant_listMod->save();
                if($result) {
                    $this->success('操作成功！', U('consultant_list/edit_consultant_listdesign', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
