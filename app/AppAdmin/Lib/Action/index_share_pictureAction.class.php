 ...
 *
 */
class index_share_pictureAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $index_share_pictureMod = M('index_share_picture');
            $sql=" id in (".$in.") ";
            $index_share_pictureMod->where($sql)->delete();

            $this->success('删除成功', U('index_share_picture/listing'));
            exit;
        }
        ...
        $this->ModManager = M('index_share_picture');
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('index_share_picture', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageMenu = array(
			array( U('index_share_picture/create'), L('添加重点分享图集') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkindex_share_pictureData_Post();

            if (false === $index_share_pictureMod->create()) {
                $this->error($module->getError());
            }

            if($index_share_pictureMod->create()) {

                //$rst=$this->Checkindex_share_pictureData_Mod($index_share_pictureMod);
                $index_share_pictureMod->create_time=time();
                $index_share_pictureMod->modify_time=time();

                $result =   $index_share_pictureMod->add();
                if($result) {
                    $this->success('操作成功！', U('index_share_picture/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($index_share_pictureMod->getError());
            }
            ...
            $PageMenu = array(
                array( U('index_share_picture/listing'), L('重点分享图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkindex_share_pictureData_Post($id);

	        $index_share_pictureMod = M('index_share_picture');

	        if($index_share_pictureMod->create()) {
	        	
                //$rst=$this->Checkindex_share_pictureData_Mod($index_share_pictureMod,$id);
                $index_share_pictureMod->modify_time=time();

	            $result =   $index_share_pictureMod->save();
	            if($result) {
	                $this->success('操作成功！', U('index_share_picture/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
	        ...
	        $index_share_pictureMod = M('index_share_picture');
		    // 读取数据
		    $data =   $index_share_pictureMod->find($id);
		    if($data) {
	...
	{
		$module = $UserMod = M('index_share_picture');
...
*/
	...
	private function Checkindex_share_pictureData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkindex_share_pictureData_Mod(&$User, $user_id=0){
...
/*重点分享图集图集*/
    ...
    public function listing_index_share_picturephoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $index_share_pictureMod = M('index_share_picturephoto');
            $sql=" id in (".$in.") ";
            $index_share_pictureMod->where($sql)->delete();

            $this->success('删除成功', U('index_share_picture/listing_index_share_picturephoto'));
            exit;
        }
        ...
        $this->ModManager = M('index_share_picturephoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('index_share_picturephoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('重点分享图集图集列表');
        $PageMenu = array(
            array( U('index_share_picture/create_index_share_picturephoto'), L('添加重点分享图集图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_index_share_picturephoto()
    {
        $module = $UserMod = M('index_share_picturephoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_index_share_picturephoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加重点分享图集图集');
            $PageMenu = array(
                array( U('index_share_picture/listing_index_share_picturephoto'), L('重点分享图集图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_index_share_picturephoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($index_share_pictureMod->create()) {
                ...
                $result =   $index_share_pictureMod->save();
                if($result) {
                    $this->success('操作成功！', U('index_share_picture/edit_index_share_picturephoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*重点分享图集行程*/
    ...
    public function listing_index_share_picturecircle()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $index_share_pictureMod = M('index_share_picturecircle');
            $sql=" id in (".$in.") ";
            $index_share_pictureMod->where($sql)->delete();

            $this->success('删除成功', U('index_share_picture/listing_index_share_picturecircle'));
            exit;
        }
        ...
        $this->ModManager = M('index_share_picturecircle');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('index_share_picturecircle', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('重点分享图集行程列表');
        $PageMenu = array(
            array( U('index_share_picture/create_index_share_picturecircle'), L('添加重点分享图集行程') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_index_share_picturecircle()
    {
        $module = $UserMod = M('index_share_picturecircle');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_index_share_picturecircle()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加重点分享图集行程');
            $PageMenu = array(
                array( U('index_share_picture/listing_index_share_picturecircle'), L('重点分享图集行程列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_index_share_picturecircle()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($index_share_pictureMod->create()) {
                ...
                $result =   $index_share_pictureMod->save();
                if($result) {
                    $this->success('操作成功！', U('index_share_picture/edit_index_share_picturecircle', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
