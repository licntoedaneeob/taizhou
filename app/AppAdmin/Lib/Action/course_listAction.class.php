 ...
 *
 */
class course_listAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $course_listMod = M('course_list');
            $sql=" id in (".$in.") ";
            $course_listMod->where($sql)->delete();

            $this->success('删除成功', U('course_list/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('course_list', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('课程列表');
		$PageMenu = array(
			array( U('course_list/create'), L('添加课程') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkcourse_listData_Post();

            if (false === $course_listMod->create()) {
                $this->error($module->getError());
            }

            if($course_listMod->create()) {

                //$rst=$this->Checkcourse_listData_Mod($course_listMod);
                $course_listMod->create_time=time();
                $course_listMod->modify_time=time();

                $result =   $course_listMod->add();
                if($result) {
                    $this->success('操作成功！', U('course_list/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($course_listMod->getError());
            }
            ...
            $PageTitle = L('添加课程');
            $PageMenu = array(
                array( U('course_list/listing'), L('课程列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkcourse_listData_Post($id);

	        $course_listMod = M('course_list');

	        if($course_listMod->create()) {
	        	
                //$rst=$this->Checkcourse_listData_Mod($course_listMod,$id);
                $course_listMod->modify_time=time();

	            $result =   $course_listMod->save();
	            if($result) {
	                $this->success('操作成功！', U('course_list/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function Checkcourse_listData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkcourse_listData_Mod(&$User, $user_id=0){
...
/*课程图集*/
    ...
    public function listing_course_listphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $course_listMod = M('course_listphoto');
            $sql=" id in (".$in.") ";
            $course_listMod->where($sql)->delete();

            $this->success('删除成功', U('course_list/listing_course_listphoto'));
            exit;
        }
        ...
        $this->ModManager = M('course_listphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('course_listphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('课程图集列表');
        $PageMenu = array(
            array( U('course_list/create_course_listphoto'), L('添加课程图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_course_listphoto()
    {
        $module = $UserMod = M('course_listphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_course_listphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加课程图集');
            $PageMenu = array(
                array( U('course_list/listing_course_listphoto'), L('课程图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_course_listphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($course_listMod->create()) {
                ...
                $result =   $course_listMod->save();
                if($result) {
                    $this->success('操作成功！', U('course_list/edit_course_listphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*课程设计*/
    ...
    public function listing_course_listdesign()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $course_listMod = M('course_listdesign');
            $sql=" id in (".$in.") ";
            $course_listMod->where($sql)->delete();

            $this->success('删除成功', U('course_list/listing_course_listdesign'));
            exit;
        }
        ...
        $this->ModManager = M('course_listdesign');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('course_listdesign', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('课程图集列表');
        $PageMenu = array(
            array( U('course_list/create_course_listdesign'), L('添加课程图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_course_listdesign()
    {
        $module = $UserMod = M('course_listdesign');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_course_listdesign()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加课程图集');
            $PageMenu = array(
                array( U('course_list/listing_course_listdesign'), L('课程图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_course_listdesign()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($course_listMod->create()) {
                ...
                $result =   $course_listMod->save();
                if($result) {
                    $this->success('操作成功！', U('course_list/edit_course_listdesign', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
            ...
            do{
                ...
                if ($toShow['banner'][$k]['status_pay']=="2"){
                    ...
                    $output .= $toShow['banner'][$k]['id']
                    	...
                    	.$expstr.$toShow['banner'][$k]['course_name']
                    	.$expstr.$toShow['banner'][$k]['course_other']
