 ...
 *
 */
class clubAction extends TAction
{
	...
	public function listing()
	{
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $clubMod = M('club');
            $sql=" id in (".$in.") ";
            $clubMod->where($sql)->delete();

            $this->success('删除成功', U('club/listing'));
            exit;
        }
		...
		if( $f_search != '' ){
			...
			elseif($filter_fieldname==''){
				$sqlWhere .= " and (realname like '%". $this->fixSQL($f_search)."%' or email like '%". $this->fixSQL($f_search)."%' or mobile like '%". $this->fixSQL($f_search)."%') ";
			}
			else{
			}
		}
		...
		///获取列表数据集
		$rst=$this->GeneralActionForListing('club', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('媒体俱乐部列表');
		$PageMenu = array(
			//array( U('club/create'), L('添加媒体俱乐部') ),
            array( U('club/export'), L('导出媒体俱乐部') ),
		);
    ...
    public function export()
    {
        ...
        $expstr="\t";
        $expenter="\t\n";
        $output .= "媒体俱乐部ID编号".$expstr."姓名".$expstr."邮箱"
            .$expstr."所在媒体".$expstr."职位"
            .$expstr."手机".$expstr."办公电话"
            .$expstr."社交媒体（sina）".$expstr."社交媒体（qq）".$expstr."社交媒体（weixin）".$expstr."社交媒体（其他）"
            .$expstr."联系地址".$expstr."感兴趣的领域".$expstr."感兴趣的新闻话题".$expstr."建议意见"
            .$expstr."创建时间"
            .$expenter;
        ...
        if (!empty($toShow['banner'])){
            ...
            do{
                    ...
                    $output .= $toShow['banner'][$k]['id'].$expstr.$toShow['banner'][$k]['realname'].$expstr.$toShow['banner'][$k]['email']
                        .$expstr.$toShow['banner'][$k]['jobmedia'].$expstr.$toShow['banner'][$k]['zhiwei']
                        .$expstr.$toShow['banner'][$k]['mobile'].$expstr.$toShow['banner'][$k]['telphone']
                        .$expstr.$toShow['banner'][$k]['snsmedia_sina'].$expstr.$toShow['banner'][$k]['snsmedia_qq'].$expstr.$toShow['banner'][$k]['snsmedia_weixin'].$expstr.$toShow['banner'][$k]['snsmedia_other']
                        .$expstr.$toShow['banner'][$k]['address'].$expstr.$toShow['banner'][$k]['interest_area'].$expstr.$toShow['banner'][$k]['interest_news'].$expstr.$toShow['banner'][$k]['suggest']
...
//直接从utf-8转,这个貌似不灵...
	...
	public function create()
	{
        ...
        //echo "<pre>";print_r($_POST);exit;
			...
			$rst=$this->CheckclubData_Post();
			
			if (false === $clubMod->create()) {
				$this->error($module->getError());
			}
			
	        if($clubMod->create()) {

	        	//echo "<pre>";print_r($clubMod);exit;

	        	//使用 $clubMod->email
        		$rst=$this->CheckclubData_Mod($clubMod);
	        	$clubMod->create_time=time();
	        	$clubMod->password=md5($clubMod->password);
			...
			$PageTitle = L('添加媒体俱乐部');
			$PageMenu = array(
					array( U('club/listing'), L('媒体俱乐部列表') ),
			);
			$this->setPagething( $PageTitle, $PageMenu, true);
			$this->display();
		}
	...
	public function edit()
	{
    	...
    	///注意：老媒体俱乐部媒体俱乐部已填写情况下不允许修改媒体俱乐部名，媒体俱乐部名空可设置，需检查唯一
    	///email 改变的话 还要检查 email 不重复

		if(isset($_POST['dosubmit'])){
			$id = intval($this->REQUEST('id'), 0);

            $rst=$this->CheckclubData_Post($id);
	        ...
	        if($clubMod->create()) {
                ...
                $rst=$this->CheckclubData_Mod($clubMod,$id);
                $clubMod->modify_time=time();

	            $result =   $clubMod->save();
	            if($result) {
	                $this->success('操作成功！', U('club/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
	        ...
	        $clubMod = M('club');
		    // 读取数据
		    $data =   $clubMod->find($id);
		    if($data) {
		        $this->record = $data;// 模板变量赋值
		    }else{
		        $this->error('媒体俱乐部数据读取错误');
		    }
...
*/
	...
	private function CheckclubData_Post($club_id=0){
		///检查 $_POST 提交数据
		
			$clubMod = M('club');

			$result = $clubMod->where("clubname='%s' and id!=%d ", $_POST['clubname'], $club_id )->count();
			if($result>0){
            $this->error(L('存在重复的媒体俱乐部名'));
            }
            
			$result = $clubMod->where("email='%s' and id!=%d ", $_POST['email'], $club_id)->count();
			if($result>0){
	...
	private function CheckclubData_Mod(&$club, $club_id=0){
		///检查 $club 模型数据。$club->email
		
			$clubMod = M('club');
			
			$result = $clubMod->where("clubname='%s' and id!=%d ", $club->clubname, $club_id )->count();
			if($result>0){
            $this->error(L('存在重复的媒体俱乐部名'));
            }
            
			$result = $clubMod->where("email='%s' and id!=%d ", $club->email, $club_id)->count();
			if($result>0){
            $this->error(L('存在重复的邮箱'));
            }
