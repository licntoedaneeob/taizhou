 ...
 *
 */
class worksAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $worksMod = M('works');
            $sql=" id in (".$in.") ";
            $worksMod->where($sql)->delete();

            $this->success('删除成功', U('works/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('works', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('创意作品列表');
		$PageMenu = array(
			array( U('works/create'), L('添加创意作品') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckworksData_Post();

            if (false === $worksMod->create()) {
                $this->error($module->getError());
            }

            if($worksMod->create()) {

                //$rst=$this->CheckworksData_Mod($worksMod);
                $worksMod->create_time=time();
                $worksMod->modify_time=time();
            ...
            $PageTitle = L('添加创意作品');
            $PageMenu = array(
                array( U('works/listing'), L('创意作品列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckworksData_Post($id);

	        $worksMod = M('works');

	        if($worksMod->create()) {
	        	
                //$rst=$this->CheckworksData_Mod($worksMod,$id);
                $worksMod->modify_time=time();

	            $result =   $worksMod->save();
	            if($result) {
	                $this->success('操作成功！', U('works/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckworksData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckworksData_Mod(&$User, $user_id=0){
...
/*创意作品图集*/
    ...
    public function listing_worksphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $worksMod = M('worksphoto');
            $sql=" id in (".$in.") ";
            $worksMod->where($sql)->delete();

            $this->success('删除成功', U('works/listing_worksphoto'));
            exit;
        }
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('worksphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('创意作品图集列表');
        $PageMenu = array(
            array( U('works/create_worksphoto'), L('添加创意作品图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_worksphoto()
    {
        $module = $UserMod = M('worksphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_worksphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加创意作品图集');
            $PageMenu = array(
                array( U('works/listing_worksphoto'), L('创意作品图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_worksphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($worksMod->create()) {
                ...
                $result =   $worksMod->save();
                if($result) {
                    $this->success('操作成功！', U('works/edit_worksphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
