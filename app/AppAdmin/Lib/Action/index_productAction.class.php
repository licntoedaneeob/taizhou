 ...
 *
 */
class index_productAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $index_productMod = M('index_product');
            $sql=" id in (".$in.") ";
            $index_productMod->where($sql)->delete();

            $this->success('删除成功', U('index_product/listing'));
            exit;
        }
        ...
        $this->ModManager = M('index_product');
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('index_product', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('商品列表');
		$PageMenu = array(
			array( U('index_product/create'), L('添加商品') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkindex_productData_Post();

            if (false === $index_productMod->create()) {
                $this->error($module->getError());
            }

            if($index_productMod->create()) {

                //$rst=$this->Checkindex_productData_Mod($index_productMod);
                $index_productMod->create_time=time();
                $index_productMod->modify_time=time();

                $result =   $index_productMod->add();
                if($result) {
                    $this->success('操作成功！', U('index_product/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($index_productMod->getError());
            }
            ...
            $PageTitle = L('添加商品');
            $PageMenu = array(
                array( U('index_product/listing'), L('商品列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkindex_productData_Post($id);

	        $index_productMod = M('index_product');

	        if($index_productMod->create()) {
	        	
                //$rst=$this->Checkindex_productData_Mod($index_productMod,$id);
                $index_productMod->modify_time=time();

	            $result =   $index_productMod->save();
	            if($result) {
	                $this->success('操作成功！', U('index_product/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
	...
	{
		$module = $UserMod = M('index_product');
...
*/
	...
	private function Checkindex_productData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkindex_productData_Mod(&$User, $user_id=0){
...
/*商品图集*/
    ...
    public function listing_index_productphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $index_productMod = M('index_productphoto');
            $sql=" id in (".$in.") ";
            $index_productMod->where($sql)->delete();

            $this->success('删除成功', U('index_product/listing_index_productphoto'));
            exit;
        }
        ...
        $this->ModManager = M('index_productphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('index_productphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('商品图集列表');
        $PageMenu = array(
            array( U('index_product/create_index_productphoto'), L('添加商品图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_index_productphoto()
    {
        $module = $UserMod = M('index_productphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_index_productphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加商品图集');
            $PageMenu = array(
                array( U('index_product/listing_index_productphoto'), L('商品图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_index_productphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($index_productMod->create()) {
                ...
                $result =   $index_productMod->save();
                if($result) {
                    $this->success('操作成功！', U('index_product/edit_index_productphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*商品行程*/
    ...
    public function listing_index_productcircle()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $index_productMod = M('index_productcircle');
            $sql=" id in (".$in.") ";
            $index_productMod->where($sql)->delete();

            $this->success('删除成功', U('index_product/listing_index_productcircle'));
            exit;
        }
        ...
        $this->ModManager = M('index_productcircle');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('index_productcircle', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('商品行程列表');
        $PageMenu = array(
            array( U('index_product/create_index_productcircle'), L('添加商品行程') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_index_productcircle()
    {
        $module = $UserMod = M('index_productcircle');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_index_productcircle()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加商品行程');
            $PageMenu = array(
                array( U('index_product/listing_index_productcircle'), L('商品行程列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_index_productcircle()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($index_productMod->create()) {
                ...
                $result =   $index_productMod->save();
                if($result) {
                    $this->success('操作成功！', U('index_product/edit_index_productcircle', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
