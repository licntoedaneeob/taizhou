	...
	public function listing()
	{
        ...
        */
		...
		if( $f_search != '' ){
			...
			elseif($filter_fieldname=='team_running_group_name'){
				$sqlWhere .= " and (team_running_group_name like '%". $this->fixSQL($f_search)."%') ";
			}
			elseif($filter_fieldname=='invit_code'){
				$sqlWhere .= " and (invit_code like '%". $this->fixSQL($f_search)."%') ";
			}
			elseif($filter_fieldname==''){
				$sqlWhere .= " and (order_no like '%". $this->fixSQL($f_search)."%' or trade_no like '%". $this->fixSQL($f_search)."%' or mobile like '%". $this->fixSQL($f_search)."%' or realname like '%". $this->fixSQL($f_search)."%' or id_number like '%". $this->fixSQL($f_search)."%' or match_code like '%". $this->fixSQL($f_search)."%' or team_running_group_name like '%". $this->fixSQL($f_search)."%' or invit_code like '%". $this->fixSQL($f_search)."%' )";
			}
			else{
			}
		}
		...
		*/
		...
		//报名模式：个人/团队
		$filter_team_mode = $this->REQUEST('_filter_team_mode');
		if( $filter_team_mode==='' ){
		}
		elseif( $filter_team_mode=='1' ){
			$sqlWhere .= " and team_id=0 ";
		}
		elseif( $filter_team_mode=='2' ){
			$sqlWhere .= " and team_id>0 ";
		}
		else{
		}
		...
		$filter_is_match_code = $this->REQUEST('_filter_is_match_code');
		if( $filter_is_match_code==='' ){
		}
		elseif( $filter_is_match_code=='1' ){
			$sqlWhere .= " and match_code!='' ";
		}
		elseif( $filter_is_match_code=='2' ){
			$sqlWhere .= " and match_code='' ";
		}
		else{
		}
		...
		$this->assign('filter_team_mode',  $filter_team_mode);
		$this->assign('filter_is_cert_medical',  $filter_is_cert_medical);
		$this->assign('filter_is_match_code',  $filter_is_match_code);
		...
		*/
		...
		if(isset($_POST['is_export']) && $_POST['is_export']==1){
            ...
            ->setCellValue('B'.$i, '报名类型(1代表全程马拉松，2代表半程马拉松，3代表迷你马拉松)')
            ->setCellValue('C'.$i, '姓名')
            ...
            ->setCellValue('Q'.$i, '是否台州户籍(1是，2否)')
            
            ->setCellValue('R'.$i, '报名申请时间')
            
            ->setCellValue('S'.$i, '体检证明')
            ->setCellValue('T'.$i, '完赛证书成绩')
            ->setCellValue('U'.$i, '成绩证明')
            
            ->setCellValue('V'.$i, '订单号')
            ->setCellValue('W'.$i, '交易号')
            ->setCellValue('X'.$i, '总金额')
            ->setCellValue('Y'.$i, '支付方式(0代表未定义，1代表支付宝，2代表微信，9代表线下支付)')
            ->setCellValue('Z'.$i, '支付状态(0代表未支付，1代表已支付，2代表待确认)')
            ->setCellValue('AA'.$i, '支付时间')
            
            ->setCellValue('AB'.$i, '参赛号')
            
            ->setCellValue('AC'.$i, '审核状态(0待审核，1审核通过，2审核拒绝)')
            ->setCellValue('AD'.$i, '审核备注')
            
            ->setCellValue('AE'.$i, '报名模式')
            ->setCellValue('AF'.$i, '领队姓名')
            ->setCellValue('AG'.$i, '领队证件号')
            ->setCellValue('AH'.$i, '领队手机号')
            ->setCellValue('AI'.$i, '团队名称')
            ->setCellValue('AJ'.$i, '领队email')
            ->setCellValue('AK'.$i, '领队联系地址')
            ->setCellValue('AL'.$i, '参队人数')
            
            ->setCellValue('AM'.$i, '民族')
            ->setCellValue('AN'.$i, '工作单位')
            ->setCellValue('AO'.$i, '单位性质')
            ->setCellValue('AP'.$i, '职位')
            ->setCellValue('AQ'.$i, '学历')
            ->setCellValue('AR'.$i, '月收入水平')
            ->setCellValue('AS'.$i, '身高(cm)')
            ->setCellValue('AT'.$i, '体重(Kg)')
            
            ->setCellValue('AU'.$i, '监护人证件类型(1身份证，2军官证，3护照，4台胞证，5回乡证)')
            ->setCellValue('AV'.$i, '监护人证件号码')
            
            //->setCellValue('AE'.$i, '抽签状态(0待抽签，1已中签，2未中签)')
            ...
            ;
  			...
  			$status_attach_reason=str_replace("\r\n"," [Enter] ",$status_attach_reason);
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                	...
                	->setCellValueExplicit('R'.$i, $row['addtime_apply'], PHPExcel_Cell_DataType::TYPE_STRING)
                	
                	->setCellValueExplicit('S'.$i, $cert_medical, PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('T'.$i, $row['best_chengji_score'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('U'.$i, $cert_chengji, PHPExcel_Cell_DataType::TYPE_STRING)
                	
                	
                	->setCellValueExplicit('V'.$i, $row['order_no'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('W'.$i, $row['trade_no'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('X'.$i, $row['amount_total'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('Y'.$i, $row['payMode'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('Z'.$i, $row['isPay'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AA'.$i, $row['payDateTime'], PHPExcel_Cell_DataType::TYPE_STRING)
                	
                	->setCellValueExplicit('AB'.$i, $row['match_code'], PHPExcel_Cell_DataType::TYPE_STRING)
                	
                	->setCellValueExplicit('AC'.$i, $row['status_attach'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AD'.$i, $status_attach_reason, PHPExcel_Cell_DataType::TYPE_STRING)
                	
                	
                	->setCellValueExplicit('AE'.$i, $team_mode, PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AF'.$i, $row['team_realname'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AG'.$i, $row['team_id_number'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AH'.$i, $row['team_mobile'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AI'.$i, $row['team_running_group_name'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AJ'.$i, $row['team_email'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AK'.$i, $row['team_address'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AL'.$i, $row['team_renshu'], PHPExcel_Cell_DataType::TYPE_STRING)
                	
                	->setCellValueExplicit('AM'.$i, $row['minzu'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AN'.$i, $row['danwei'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AO'.$i, $row['danwei_xingzhi'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AP'.$i, $row['zhiwei'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AQ'.$i, $row['xueli'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AR'.$i, $row['yueshouru'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AS'.$i, $row['shengao'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AT'.$i, $row['tizhong'], PHPExcel_Cell_DataType::TYPE_STRING)
                	
                	->setCellValueExplicit('AU'.$i, $row['id_type_guard'], PHPExcel_Cell_DataType::TYPE_STRING)
                	->setCellValueExplicit('AV'.$i, $row['id_number_guard'], PHPExcel_Cell_DataType::TYPE_STRING)
                	
                	//->setCellValueExplicit('AE'.$i, $row['status_apply'], PHPExcel_Cell_DataType::TYPE_STRING)
                	//->setCellValueExplicit('Q'.$i, $row['renshou_zengxian'], PHPExcel_Cell_DataType::TYPE_STRING)
	        ...
	        */
	        ...
	        //header('Content-Disposition: attachment; filename="报名人员列表.csv"');  
		...
		if(isset($_POST['dosubmit'])){
	        ...
	        if($orderMod->create()) {
	            ...
	            if($result) {
	            	...
	            	/*
	            	//设为已中签
	            	if(  ($order_info['status_apply']==0 || $order_info['status_apply']==2) && $_POST['status_apply']==1     ){
						...
						$msg_body='尊敬的'.$order_info['realname'].'，恭喜您已中签东风日产2017成都国际马拉松赛马拉松项目，请于8月20日17:00前填写摆渡信息，感谢您的支持和参与！【成都国际马拉松组委会】';
						}
						if($order_info['cat_id']==2){
						$msg_body='尊敬的'.$order_info['realname'].'，恭喜您已中签东风日产2017成都国际马拉松赛半程马拉松项目，请于8月20日17:00前填写摆渡信息，感谢您的支持和参与！【成都国际马拉松组委会】';
						}
						if($order_info['cat_id']==3){
						$msg_body='尊敬的'.$order_info['realname'].'，恭喜您已中签东风日产2017成都国际马拉松赛迷你马拉松项目，请于8月20日17:00前填写摆渡信息，感谢您的支持和参与！【成都国际马拉松组委会】';
						}
	            	...
	            	*/
	            	...
	            	if( ($order_info['status_attach']==0 || $order_info['status_attach']==2) && $_POST['status_attach']==1){
						...
						$msg_body='尊敬的'.$order_info['realname'].'，恭喜您通过东风日产2017成都国际马拉松赛的参赛资料审核并报名成功，您可于9月8日10:00查询参赛号码，感谢您的支持和参与！【成都国际马拉松组委会】';
						}
						else{
							if( $order_info['cat_id']==4  ){
							$msg_body='尊敬的'.$order_info['realname'].'，恭喜您已成功报名东风日产2017成都马拉松赛公益跑全程马拉松，请您尽快缴纳报名费，报名以交费为准。【成都国际马拉松组委会】';
							}
							elseif( $order_info['cat_id']==5 ){
							$msg_body='尊敬的'.$order_info['realname'].'，恭喜您已成功报名东风日产2017成都马拉松赛公益跑半程马拉松，请您尽快缴纳报名费，报名以交费为准。【成都国际马拉松组委会】';
							}
							else{
							$msg_body='尊敬的'.$order_info['realname'].'，恭喜您已通过东风日产2017成都国际马拉松赛的参赛资料审核，撒花~~请您于8月21日10:00至8月23日17:00支付报名费，报名完成以支付成功为准，感谢您的支持和参与！期待在赛道上与您相遇！【成都国际马拉松组委会】';
							}
						}
						$result_email=$this->think_send_mail($to, $name, $subject, $msg_body);
...
//直接从utf-8转,这个貌似不灵...
    ...
    public function get_cityarea(){
    	$CityMod = M('guoji');
        $parent_list = $CityMod->field('id,guoji_name')->where(" 1 " )->order('sort asc,guoji_name_abbr asc')->select();
        //echo "<pre>";print_r($parent_list);exit;
