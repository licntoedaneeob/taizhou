 ...
 *
 */
class share_listAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $share_listMod = M('share_list');
            $sql=" id in (".$in.") ";
            $share_listMod->where($sql)->delete();

            $this->success('删除成功', U('share_list/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('share_list', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('斌友列表');
		$PageMenu = array(
			array( U('share_list/create'), L('添加斌友') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkshare_listData_Post();

            if (false === $share_listMod->create()) {
                $this->error($module->getError());
            }

            if($share_listMod->create()) {

                //$rst=$this->Checkshare_listData_Mod($share_listMod);
                $share_listMod->create_time=time();
                $share_listMod->modify_time=time();

                $result =   $share_listMod->add();
                if($result) {
                    $this->success('操作成功！', U('share_list/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($share_listMod->getError());
            }
            ...
            $PageTitle = L('添加斌友');
            $PageMenu = array(
                array( U('share_list/listing'), L('斌友列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkshare_listData_Post($id);

	        $share_listMod = M('share_list');

	        if($share_listMod->create()) {
	        	
                //$rst=$this->Checkshare_listData_Mod($share_listMod,$id);
                $share_listMod->modify_time=time();

	            $result =   $share_listMod->save();
	            if($result) {
	                $this->success('操作成功！', U('share_list/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function Checkshare_listData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkshare_listData_Mod(&$User, $user_id=0){
...
/*斌友图集*/
    ...
    public function listing_share_listphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $share_listMod = M('share_listphoto');
            $sql=" id in (".$in.") ";
            $share_listMod->where($sql)->delete();

            $this->success('删除成功', U('share_list/listing_share_listphoto'));
            exit;
        }
        ...
        $this->ModManager = M('share_listphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('share_listphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('斌友图集列表');
        $PageMenu = array(
            array( U('share_list/create_share_listphoto'), L('添加斌友图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_share_listphoto()
    {
        $module = $UserMod = M('share_listphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_share_listphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加斌友图集');
            $PageMenu = array(
                array( U('share_list/listing_share_listphoto'), L('斌友图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_share_listphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($share_listMod->create()) {
                ...
                $result =   $share_listMod->save();
                if($result) {
                    $this->success('操作成功！', U('share_list/edit_share_listphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*斌友设计*/
    ...
    public function listing_share_listdesign()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $share_listMod = M('share_listdesign');
            $sql=" id in (".$in.") ";
            $share_listMod->where($sql)->delete();

            $this->success('删除成功', U('share_list/listing_share_listdesign'));
            exit;
        }
        ...
        $this->ModManager = M('share_listdesign');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('share_listdesign', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('斌友图集列表');
        $PageMenu = array(
            array( U('share_list/create_share_listdesign'), L('添加斌友图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_share_listdesign()
    {
        $module = $UserMod = M('share_listdesign');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_share_listdesign()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加斌友图集');
            $PageMenu = array(
                array( U('share_list/listing_share_listdesign'), L('斌友图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_share_listdesign()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($share_listMod->create()) {
                ...
                $result =   $share_listMod->save();
                if($result) {
                    $this->success('操作成功！', U('share_list/edit_share_listdesign', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
