 ...
 *
 */
class productAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $productMod = M('product');
            $sql=" id in (".$in.") ";
            $productMod->where($sql)->delete();

            $this->success('删除成功', U('product/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('product', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('产品专区列表');
		$PageMenu = array(
			array( U('product/create'), L('添加产品专区') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckproductData_Post();

            if (false === $productMod->create()) {
                $this->error($module->getError());
            }

            if($productMod->create()) {

                //$rst=$this->CheckproductData_Mod($productMod);
                $productMod->create_time=time();
                $productMod->modify_time=time();

                $result =   $productMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
            ...
            $PageTitle = L('添加产品专区');
            $PageMenu = array(
                array( U('product/listing'), L('产品专区列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckproductData_Post($id);

	        $productMod = M('product');

	        if($productMod->create()) {
	        	
                //$rst=$this->CheckproductData_Mod($productMod,$id);
                $productMod->modify_time=time();

	            $result =   $productMod->save();
	            if($result) {
	                $this->success('操作成功！', U('product/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckproductData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckproductData_Mod(&$User, $user_id=0){
...
/*产品专区图集*/
    ...
    public function listing_productphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $productMod = M('productphoto');
            $sql=" id in (".$in.") ";
            $productMod->where($sql)->delete();

            $this->success('删除成功', U('product/listing_productphoto'));
            exit;
        }
        ...
        $this->ModManager = M('productphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('productphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageMenu = array(
            array( U('product/create_productphoto'), L('添加产品专区图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_productphoto()
    {
        $module = $UserMod = M('productphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_productphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageMenu = array(
                array( U('product/listing_productphoto'), L('产品专区图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_productphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($productMod->create()) {
                ...
                $result =   $productMod->save();
                if($result) {
                    $this->success('操作成功！', U('product/edit_productphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
