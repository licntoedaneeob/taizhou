 ...
 *
 */
class news_classAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $news_classMod = M('news_class');
            $sql=" id in (".$in.") ";
            $news_classMod->where($sql)->delete();

            $this->success('删除成功', U('news_class/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('news_class', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('文章类型列表');
		$PageMenu = array(
			array( U('news_class/create'), L('添加文章类型') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checknews_classData_Post();

            if (false === $news_classMod->create()) {
                $this->error($module->getError());
            }

            if($news_classMod->create()) {

                //$rst=$this->Checknews_classData_Mod($news_classMod);
                $news_classMod->create_time=time();
                $news_classMod->modify_time=time();

                $result =   $news_classMod->add();
                if($result) {
                    $this->success('操作成功！', U('news_class/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($news_classMod->getError());
            }
            ...
            $PageTitle = L('添加文章类型');
            $PageMenu = array(
                array( U('news_class/listing'), L('文章类型列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checknews_classData_Post($id);

	        $news_classMod = M('news_class');

	        if($news_classMod->create()) {
	        	
                //$rst=$this->Checknews_classData_Mod($news_classMod,$id);
                $news_classMod->modify_time=time();

	            $result =   $news_classMod->save();
	            if($result) {
	                $this->success('操作成功！', U('news_class/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function Checknews_classData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checknews_classData_Mod(&$User, $user_id=0){
...
/*文章类型图集*/
    ...
    public function listing_news_classphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $news_classMod = M('news_classphoto');
            $sql=" id in (".$in.") ";
            $news_classMod->where($sql)->delete();

            $this->success('删除成功', U('news_class/listing_news_classphoto'));
            exit;
        }
        ...
        $this->ModManager = M('news_classphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('news_classphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('文章类型图集列表');
        $PageMenu = array(
            array( U('news_class/create_news_classphoto'), L('添加文章类型图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_news_classphoto()
    {
        $module = $UserMod = M('news_classphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_news_classphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加文章类型图集');
            $PageMenu = array(
                array( U('news_class/listing_news_classphoto'), L('文章类型图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_news_classphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($news_classMod->create()) {
                ...
                $result =   $news_classMod->save();
                if($result) {
                    $this->success('操作成功！', U('news_class/edit_news_classphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*文章类型设计*/
    ...
    public function listing_news_classdesign()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $news_classMod = M('news_classdesign');
            $sql=" id in (".$in.") ";
            $news_classMod->where($sql)->delete();

            $this->success('删除成功', U('news_class/listing_news_classdesign'));
            exit;
        }
        ...
        $this->ModManager = M('news_classdesign');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('news_classdesign', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('文章类型图集列表');
        $PageMenu = array(
            array( U('news_class/create_news_classdesign'), L('添加文章类型图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_news_classdesign()
    {
        $module = $UserMod = M('news_classdesign');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_news_classdesign()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加文章类型图集');
            $PageMenu = array(
                array( U('news_class/listing_news_classdesign'), L('文章类型图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_news_classdesign()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($news_classMod->create()) {
                ...
                $result =   $news_classMod->save();
                if($result) {
                    $this->success('操作成功！', U('news_class/edit_news_classdesign', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
