 ...
 *
 */
class energyAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $energyMod = M('energy');
            $sql=" id in (".$in.") ";
            $energyMod->where($sql)->delete();

            $this->success('删除成功', U('energy/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('energy', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('能量志列表');
		$PageMenu = array(
			array( U('energy/create'), L('添加能量志') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckenergyData_Post();

            if (false === $energyMod->create()) {
                $this->error($module->getError());
            }

            if($energyMod->create()) {

                //$rst=$this->CheckenergyData_Mod($energyMod);
                $energyMod->create_time=time();
                $energyMod->modify_time=time();

                $result =   $energyMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
            ...
            $PageTitle = L('添加能量志');
            $PageMenu = array(
                array( U('energy/listing'), L('能量志列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckenergyData_Post($id);

	        $energyMod = M('energy');

	        if($energyMod->create()) {
	        	
                //$rst=$this->CheckenergyData_Mod($energyMod,$id);
                $energyMod->modify_time=time();

	            $result =   $energyMod->save();
	            if($result) {
	                $this->success('操作成功！', U('energy/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckenergyData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckenergyData_Mod(&$User, $user_id=0){
...
/*能量志图集*/
    ...
    public function listing_energyphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $energyMod = M('energyphoto');
            $sql=" id in (".$in.") ";
            $energyMod->where($sql)->delete();

            $this->success('删除成功', U('energy/listing_energyphoto'));
            exit;
        }
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('energyphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('能量志图集列表');
        $PageMenu = array(
            array( U('energy/create_energyphoto'), L('添加能量志图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_energyphoto()
    {
        $module = $UserMod = M('energyphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_energyphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加能量志图集');
            $PageMenu = array(
                array( U('energy/listing_energyphoto'), L('能量志图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_energyphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($energyMod->create()) {
                ...
                $result =   $energyMod->save();
                if($result) {
                    $this->success('操作成功！', U('energy/edit_energyphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
