 ...
 *
 */
class activitymbAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $activitymbMod = M('activitymb');
            $sql=" id in (".$in.") ";
            $activitymbMod->where($sql)->delete();

            $this->success('删除成功', U('activitymb/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('activitymb', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('品牌营销列表');
		$PageMenu = array(
			array( U('activitymb/create'), L('添加品牌营销') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckactivitymbData_Post();

            if (false === $activitymbMod->create()) {
                $this->error($module->getError());
            }

            if($activitymbMod->create()) {

                //$rst=$this->CheckactivitymbData_Mod($activitymbMod);
                $activitymbMod->create_time=time();
            ...
            $PageTitle = L('添加品牌营销');
            $PageMenu = array(
                array( U('activitymb/listing'), L('品牌营销列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
	        ...
	        if($activitymbMod->create()) {
	            ...
	            if($result) {
	                $this->success('操作成功！', U('activitymb/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckactivitymbData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckactivitymbData_Mod(&$User, $user_id=0){
...
/*品牌营销图集*/
    ...
    public function listing_activitymbphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $activitymbMod = M('activitymbphoto');
            $sql=" id in (".$in.") ";
            $activitymbMod->where($sql)->delete();

            $this->success('删除成功', U('activitymb/listing_activitymbphoto'));
            exit;
        }
        ...
        $this->ModManager = M('activitymbphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('activitymbphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('品牌营销图集列表');
        $PageMenu = array(
            array( U('activitymb/create_activitymbphoto'), L('添加品牌营销图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_activitymbphoto()
    {
        $module = $UserMod = M('activitymbphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_activitymbphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加品牌营销图集');
            $PageMenu = array(
                array( U('activitymb/listing_activitymbphoto'), L('品牌营销图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_activitymbphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($activitymbMod->create()) {
                ...
                $result =   $activitymbMod->save();
                if($result) {
                    $this->success('操作成功！', U('activitymb/edit_activitymbphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
