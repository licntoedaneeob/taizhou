 ...
 *
 */
class competitionAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $competitionMod = M('competition');
            $sql=" id in (".$in.") ";
            $competitionMod->where($sql)->delete();

            $this->success('删除成功', U('competition/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('competition', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('红牛赛事列表');
		$PageMenu = array(
			array( U('competition/create'), L('添加红牛赛事') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckcompetitionData_Post();

            if (false === $competitionMod->create()) {
                $this->error($module->getError());
            }

            if($competitionMod->create()) {

                //$rst=$this->CheckcompetitionData_Mod($competitionMod);
                $competitionMod->create_time=time();
                $competitionMod->modify_time=time();

                $result =   $competitionMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
                ...
                $this->error($competitionMod->getError());
            }
            ...
            $PageTitle = L('添加红牛赛事');
            $PageMenu = array(
                array( U('competition/listing'), L('红牛赛事列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckcompetitionData_Post($id);

	        $competitionMod = M('competition');

	        if($competitionMod->create()) {
	        	
                //$rst=$this->CheckcompetitionData_Mod($competitionMod,$id);
                $competitionMod->modify_time=time();

	            $result =   $competitionMod->save();
	            if($result) {
	                $this->success('操作成功！', U('competition/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckcompetitionData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckcompetitionData_Mod(&$User, $user_id=0){
...
/*红牛赛事图集*/
    ...
    public function listing_competitionphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $competitionMod = M('competitionphoto');
            $sql=" id in (".$in.") ";
            $competitionMod->where($sql)->delete();

            $this->success('删除成功', U('competition/listing_competitionphoto'));
            exit;
        }
        ...
        $this->ModManager = M('competitionphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('competitionphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('红牛赛事图集列表');
        $PageMenu = array(
            array( U('competition/create_competitionphoto'), L('添加红牛赛事图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_competitionphoto()
    {
        $module = $UserMod = M('competitionphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_competitionphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加红牛赛事图集');
            $PageMenu = array(
                array( U('competition/listing_competitionphoto'), L('红牛赛事图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_competitionphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($competitionMod->create()) {
                ...
                $result =   $competitionMod->save();
                if($result) {
                    $this->success('操作成功！', U('competition/edit_competitionphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
