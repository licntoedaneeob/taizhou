 ...
 *
 */
class index_share_commentAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $index_share_commentMod = M('index_share_comment');
            $sql=" id in (".$in.") ";
            $index_share_commentMod->where($sql)->delete();

            $this->success('删除成功', U('index_share_comment/listing'));
            exit;
        }
        ...
        $this->ModManager = M('index_share_comment');
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('index_share_comment', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('热门评论列表');
		$PageMenu = array(
			array( U('index_share_comment/create'), L('添加热门评论') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkindex_share_commentData_Post();

            if (false === $index_share_commentMod->create()) {
                $this->error($module->getError());
            }

            if($index_share_commentMod->create()) {

                //$rst=$this->Checkindex_share_commentData_Mod($index_share_commentMod);
                $index_share_commentMod->create_time=time();
                $index_share_commentMod->modify_time=time();

                $result =   $index_share_commentMod->add();
                if($result) {
                    $this->success('操作成功！', U('index_share_comment/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($index_share_commentMod->getError());
            }
            ...
            $PageTitle = L('添加热门评论');
            $PageMenu = array(
                array( U('index_share_comment/listing'), L('热门评论列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkindex_share_commentData_Post($id);

	        $index_share_commentMod = M('index_share_comment');

	        if($index_share_commentMod->create()) {
	        	
                //$rst=$this->Checkindex_share_commentData_Mod($index_share_commentMod,$id);
                $index_share_commentMod->modify_time=time();

	            $result =   $index_share_commentMod->save();
	            if($result) {
	                $this->success('操作成功！', U('index_share_comment/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
	        ...
	        $index_share_commentMod = M('index_share_comment');
		    // 读取数据
		    $data =   $index_share_commentMod->find($id);
		    if($data) {
	...
	{
		$module = $UserMod = M('index_share_comment');
...
*/
	...
	private function Checkindex_share_commentData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkindex_share_commentData_Mod(&$User, $user_id=0){
...
/*热门评论图集*/
    ...
    public function listing_index_share_commentphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $index_share_commentMod = M('index_share_commentphoto');
            $sql=" id in (".$in.") ";
            $index_share_commentMod->where($sql)->delete();

            $this->success('删除成功', U('index_share_comment/listing_index_share_commentphoto'));
            exit;
        }
        ...
        $this->ModManager = M('index_share_commentphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('index_share_commentphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('热门评论图集列表');
        $PageMenu = array(
            array( U('index_share_comment/create_index_share_commentphoto'), L('添加热门评论图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_index_share_commentphoto()
    {
        $module = $UserMod = M('index_share_commentphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_index_share_commentphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加热门评论图集');
            $PageMenu = array(
                array( U('index_share_comment/listing_index_share_commentphoto'), L('热门评论图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_index_share_commentphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($index_share_commentMod->create()) {
                ...
                $result =   $index_share_commentMod->save();
                if($result) {
                    $this->success('操作成功！', U('index_share_comment/edit_index_share_commentphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*热门评论行程*/
    ...
    public function listing_index_share_commentcircle()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $index_share_commentMod = M('index_share_commentcircle');
            $sql=" id in (".$in.") ";
            $index_share_commentMod->where($sql)->delete();

            $this->success('删除成功', U('index_share_comment/listing_index_share_commentcircle'));
            exit;
        }
        ...
        $this->ModManager = M('index_share_commentcircle');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('index_share_commentcircle', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('热门评论行程列表');
        $PageMenu = array(
            array( U('index_share_comment/create_index_share_commentcircle'), L('添加热门评论行程') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_index_share_commentcircle()
    {
        $module = $UserMod = M('index_share_commentcircle');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_index_share_commentcircle()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加热门评论行程');
            $PageMenu = array(
                array( U('index_share_comment/listing_index_share_commentcircle'), L('热门评论行程列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_index_share_commentcircle()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($index_share_commentMod->create()) {
                ...
                $result =   $index_share_commentMod->save();
                if($result) {
                    $this->success('操作成功！', U('index_share_comment/edit_index_share_commentcircle', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
