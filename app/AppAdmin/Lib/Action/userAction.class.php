...
class userAction extends TAction
{
	...
	public function listing()
	{
        ...
        */
		...
		/*
        $quanMod = M('quan_list');
        $quan_list = $quanMod->where(" status=1 " )->user('sort asc,id desc')->select();
        $quan_arr=array();
        if(!empty($quan_list)){
        ...
        */
        ...
        //获得赛事分站列表
        $userMod = M('user');
        $catalog_stage_list = $userMod->field('catalog_id,catalog_name,stage_id,stage_name')->where(" status=1 " )->group('catalog_id,stage_id')->select();
        $vals = array();
        ...
        //获得所有团队成员
        $user_teamMod = M('user_team');
        $user_team_list = $user_teamMod->field('user_id,t_realname,t_sex,t_mobile,t_id_type,t_id_number,t_birth_day')->where(" 1 " )->select();
        $user_team_arr=array();
        if(!empty($user_team_list)){
			foreach($user_team_list as $k => $v){
				$v_tmp=$v;
				$v_tmp['t_sex']=($v_tmp['t_sex']==1)?'男':'女';
				unset($v_tmp['user_id']);
				//echo "<pre>";print_r($v_tmp);exit;
				$user_team_arr[$v['user_id']]['list'][] = $v;
				$info=implode("_", $v_tmp);
				$user_team_arr[$v['user_id']]['info'][] = $info;
				
			}
		}
		//echo "<pre>";print_r($user_team_arr);exit;
        $this->assign('user_team_arr', $user_team_arr);
        
        
        
        
        //获得所有商品
        $user_productMod = M('user_product');
        $user_product_list = $user_productMod->field('user_id,product_name,sku_name,price,number,price_sub')->where(" 1 " )->select();
        $user_product_arr=array();
        if(!empty($user_product_list)){
			foreach($user_product_list as $k => $v){
				$v_tmp=$v;
				unset($v_tmp['user_id']);
				//echo "<pre>";print_r($v_tmp);exit;
				$user_product_arr[$v['user_id']]['list'][] = $v;
				$info=implode("_", $v_tmp);
				$user_product_arr[$v['user_id']]['info'][] = $info;
				
			}
		}
		//echo "<pre>";print_r($user_product_arr);exit;
        $this->assign('user_product_arr', $user_product_arr);
        */
		...
		*/
        ...
        $f_user = $this->REQUEST('_f_user', 'id');
        $fields = $this->ModManager->getDbFields();
        if( in_array($f_user, $fields) ){
            $sqluser = $f_user . ' ';
        }else{
            $sqluser = 'id ';
        }
		...
		*/
		...
		if(isset($_POST['is_export']) && $_POST['is_export']==1){
			//导出
			$rst=$this->GeneralActionForListing('user', $sqlWhere, $sqluser, '100000000', 'M');
            ...
            ->setCellValue('A'.$i, '用户流水ID编号')
            ->setCellValue('B'.$i, '报名时间')
            
            /*
            ->setCellValue('C'.$i, '用户号')
            ...
            */
            ;
  			...
  			foreach ($toShow['banner'] as $k=>$row){
                ...
                $user_id=$toShow['banner'][$k]['id'];
                
                $user_no='';
                if($toShow['banner'][$k]['user_no']!=''){
                	$user_no="'".$toShow['banner'][$k]['user_no'];
                }
                ...
                $m_info='';
                if($toShow['banner'][$k]['user_type']==2){
                	$m_info=implode("|", $user_team_arr[$user_id]['info']);
                }
                
                
                $p_info='';
                if(!empty($user_product_arr[$user_id]['info'])){
                	$p_info=implode("|", $user_product_arr[$user_id]['info']);
                }
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    /*
                    ->setCellValueExplicit('C'.$i, $row['user_no'], PHPExcel_Cell_DataType::TYPE_STRING)
	        ...
	        //搜索查询显示
			//$rst=$this->GeneralActionForListing('user', $sqlWhere, $sqluser, $page_size, 'M', false, 'catalog_id,stage_id');
			$rst=$this->GeneralActionForListing('user', $sqlWhere, $sqluser, $page_size, 'M');
	...
	{	
	    ...
	    if($info) {
	        ...
	        if($info['user_type']==2){
		        $user_teamMod = M('user_team');
		        $user_team_list = $user_teamMod->where(" user_id='".$user_id."' " )->select();
		        //echo "<pre>";print_r($user_team_list);exit;
	        }
	        $this->assign('user_team_list', $user_team_list);
	        
	        
	        $user_product_list=array();
	        $rder_productMod = M('user_product');
	        $user_product_list = $rder_productMod->where(" user_id='".$user_id."' " )->select();
	        //echo "<pre>";print_r($user_product_list);exit;
	        $this->assign('user_product_list', $user_product_list);
    ...
    {
        $CityMod = M('user');
        $toShow['banner'] = $CityMod->where(" status=1 and username!=''  " )->user('id desc')->select();
...
//echo "aaa";exit;
        //$CityMod = M('hangye');
        //$hangye_list = $CityMod->field('id,title')->where(" status=1 " )->user('sort asc')->select();
...
//直接从utf-8转,这个貌似不灵...
	...
	public function listing_import()
	{
        ...
        if(isset($_FILES['importfile']['name']) && $_FILES['importfile']['name']!=''  ){
			...
			$user_last = $userMod->where(" 1 " )->user(' id desc')->limit('0,1')->select();
			if(!empty($user_last)){
    ...
    public function getAllClassQuan(){
        ...
        $CityMod = M('quan_list');
        $parent_list = $CityMod->field('id,title,start_time,end_time')->where(" status=1 " )->user('sort asc,id desc')->select();
