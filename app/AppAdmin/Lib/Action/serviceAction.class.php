...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $serviceMod = M('service');
            $sql=" id in (".$in.") ";
            $serviceMod->where($sql)->delete();

            $this->success('删除成功', U('service/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('service', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('赛事服务列表');
		$PageMenu = array(
			//array( U('service/create'), L('添加赛事服务') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckserviceData_Post();

            if (false === $serviceMod->create()) {
                $this->error($module->getError());
            }

            if($serviceMod->create()) {

                //$rst=$this->CheckserviceData_Mod($serviceMod);
                $serviceMod->create_time=time();
                $serviceMod->modify_time=time();

                $result =   $serviceMod->add();
                if($result) {
                    $this->success('操作成功！', U('service/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($serviceMod->getError());
            }
            ...
            $PageTitle = L('添加赛事服务');
            $PageMenu = array(
                array( U('service/listing'), L('赛事服务列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckserviceData_Post($id);

	        $serviceMod = M('service');

	        if($serviceMod->create()) {
	        	
                //$rst=$this->CheckserviceData_Mod($serviceMod,$id);
                $serviceMod->modify_time=time();

	            $result =   $serviceMod->save();
	            if($result) {
	                $this->success('操作成功！', U('service/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckserviceData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckserviceData_Mod(&$User, $user_id=0){
...
/*赛事服务图集*/
    ...
    public function listing_servicephoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $serviceMod = M('servicephoto');
            $sql=" id in (".$in.") ";
            $serviceMod->where($sql)->delete();

            $this->success('删除成功', U('service/listing_servicephoto'));
            exit;
        }
        ...
        $this->ModManager = M('servicephoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('servicephoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('赛事服务图集列表');
        $PageMenu = array(
            array( U('service/create_servicephoto'), L('添加赛事服务图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_servicephoto()
    {
        $module = $UserMod = M('servicephoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_servicephoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加赛事服务图集');
            $PageMenu = array(
                array( U('service/listing_servicephoto'), L('赛事服务图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_servicephoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($serviceMod->create()) {
                ...
                $result =   $serviceMod->save();
                if($result) {
                    $this->success('操作成功！', U('service/edit_servicephoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*赛事服务设计*/
    ...
    public function listing_servicedesign()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $serviceMod = M('servicedesign');
            $sql=" id in (".$in.") ";
            $serviceMod->where($sql)->delete();

            $this->success('删除成功', U('service/listing_servicedesign'));
            exit;
        }
        ...
        $this->ModManager = M('servicedesign');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('servicedesign', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('赛事服务图集列表');
        $PageMenu = array(
            array( U('service/create_servicedesign'), L('添加赛事服务图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_servicedesign()
    {
        $module = $UserMod = M('servicedesign');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_servicedesign()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加赛事服务图集');
            $PageMenu = array(
                array( U('service/listing_servicedesign'), L('赛事服务图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_servicedesign()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($serviceMod->create()) {
                ...
                $result =   $serviceMod->save();
                if($result) {
                    $this->success('操作成功！', U('service/edit_servicedesign', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
