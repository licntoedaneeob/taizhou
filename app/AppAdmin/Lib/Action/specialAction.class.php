 ...
 *
 */
class specialAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $sql=" id in (".$in.") ";
            $specialMod->where($sql)->delete();

            $this->success('删除成功', U('special/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('special', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('首页信息表');
		$PageMenu = array(
			//array( U('special/create'), L('添加首页信息') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckspecialData_Post();

            if (false === $specialMod->create()) {
                $this->error($module->getError());
            }

            if($specialMod->create()) {

                //$rst=$this->CheckspecialData_Mod($specialMod);
                $specialMod->create_time=time();
                $specialMod->modify_time=time();

                $result =   $specialMod->add();
                if($result) {
                    $this->success('操作成功！', U('special/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($specialMod->getError());
            }
            ...
            $PageTitle = L('添加首页信息');
            $PageMenu = array(
                array( U('special/listing'), L('首页信息表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckspecialData_Post($id);

	        $specialMod = M('special');

	        if($specialMod->create()) {
	        	
                //$rst=$this->CheckspecialData_Mod($specialMod,$id);
                $specialMod->modify_time=time();

	            $result =   $specialMod->save();
	            if($result) {
	                $this->success('操作成功！', U('special/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckspecialData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckspecialData_Mod(&$User, $user_id=0){
...
/*首页信息图集*/
    ...
    public function listing_specialphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $specialMod = M('specialphoto');
            $sql=" id in (".$in.") ";
            $specialMod->where($sql)->delete();

            $this->success('删除成功', U('special/listing_specialphoto'));
            exit;
        }
        ...
        $this->ModManager = M('specialphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('specialphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('首页信息图集列表');
        $PageMenu = array(
            array( U('special/create_specialphoto'), L('添加首页信息图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_specialphoto()
    {
        $module = $UserMod = M('specialphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_specialphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加首页信息图集');
            $PageMenu = array(
                array( U('special/listing_specialphoto'), L('首页信息图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_specialphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($specialMod->create()) {
                ...
                $result =   $specialMod->save();
                if($result) {
                    $this->success('操作成功！', U('special/edit_specialphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*首页信息设计*/
    ...
    public function listing_specialdesign()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $specialMod = M('specialdesign');
            $sql=" id in (".$in.") ";
            $specialMod->where($sql)->delete();

            $this->success('删除成功', U('special/listing_specialdesign'));
            exit;
        }
        ...
        $this->ModManager = M('specialdesign');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('specialdesign', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('首页信息图集列表');
        $PageMenu = array(
            array( U('special/create_specialdesign'), L('添加首页信息图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_specialdesign()
    {
        $module = $UserMod = M('specialdesign');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_specialdesign()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加首页信息图集');
            $PageMenu = array(
                array( U('special/listing_specialdesign'), L('首页信息图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_specialdesign()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($specialMod->create()) {
                ...
                $result =   $specialMod->save();
                if($result) {
                    $this->success('操作成功！', U('special/edit_specialdesign', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
