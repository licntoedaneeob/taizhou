...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $expoMod = M('expo');
            $sql=" id in (".$in.") ";
            $expoMod->where($sql)->delete();

            $this->success('删除成功', U('expo/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('expo', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('马博会EXPO列表');
		$PageMenu = array(
			//array( U('expo/create'), L('添加马博会EXPO') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckexpoData_Post();

            if (false === $expoMod->create()) {
                $this->error($module->getError());
            }

            if($expoMod->create()) {

                //$rst=$this->CheckexpoData_Mod($expoMod);
                $expoMod->create_time=time();
                $expoMod->modify_time=time();

                $result =   $expoMod->add();
                if($result) {
                    $this->success('操作成功！', U('expo/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($expoMod->getError());
            }

        }else{

            $PageTitle = L('添加马博会EXPO');
            $PageMenu = array(
                array( U('expo/listing'), L('马博会EXPO列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckexpoData_Post($id);

	        $expoMod = M('expo');

	        if($expoMod->create()) {
	        	
                //$rst=$this->CheckexpoData_Mod($expoMod,$id);
                $expoMod->modify_time=time();

	            $result =   $expoMod->save();
	            if($result) {
	                $this->success('操作成功！', U('expo/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckexpoData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckexpoData_Mod(&$User, $user_id=0){
...
/*马博会EXPO图集*/
    ...
    public function listing_expophoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $this->success('删除成功', U('expo/listing_expophoto'));
            exit;
        }
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('expophoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('马博会EXPO图集列表');
        $PageMenu = array(
            array( U('expo/create_expophoto'), L('添加马博会EXPO图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_expophoto()
    {
        $module = $UserMod = M('expophoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_expophoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加马博会EXPO图集');
            $PageMenu = array(
                array( U('expo/listing_expophoto'), L('马博会EXPO图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_expophoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($expoMod->create()) {
                ...
                $result =   $expoMod->save();
                if($result) {
                    $this->success('操作成功！', U('expo/edit_expophoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*马博会EXPO设计*/
    ...
    public function listing_expodesign()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $expoMod = M('expodesign');
            $sql=" id in (".$in.") ";
            $expoMod->where($sql)->delete();

            $this->success('删除成功', U('expo/listing_expodesign'));
            exit;
        }
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('expodesign', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('马博会EXPO图集列表');
        $PageMenu = array(
            array( U('expo/create_expodesign'), L('添加马博会EXPO图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_expodesign()
    {
        $module = $UserMod = M('expodesign');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_expodesign()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加马博会EXPO图集');
            $PageMenu = array(
                array( U('expo/listing_expodesign'), L('马博会EXPO图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_expodesign()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($expoMod->create()) {
                ...
                $result =   $expoMod->save();
                if($result) {
                    $this->success('操作成功！', U('expo/edit_expodesign', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
