 ...
 *
 */
class product_banner_adAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_banner_adMod = M('product_banner_ad');
            $sql=" id in (".$in.") ";
            $product_banner_adMod->where($sql)->delete();

            $this->success('删除成功', U('product_banner_ad/listing'));
            exit;
        }
        ...
        $this->ModManager = M('product_banner_ad');
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('product_banner_ad', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('路线主推列表');
		$PageMenu = array(
			//array( U('product_banner_ad/create'), L('添加路线主推') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkproduct_banner_adData_Post();

            if (false === $product_banner_adMod->create()) {
                $this->error($module->getError());
            }

            if($product_banner_adMod->create()) {

                //$rst=$this->Checkproduct_banner_adData_Mod($product_banner_adMod);
                $product_banner_adMod->create_time=time();
                $product_banner_adMod->modify_time=time();

                $result =   $product_banner_adMod->add();
                if($result) {
                    $this->success('操作成功！', U('product_banner_ad/create'));
                }else{
                    $this->error('写入错误！');
                }
            }else{
                $this->error($product_banner_adMod->getError());
            }
            ...
            $PageTitle = L('添加路线主推');
            $PageMenu = array(
                array( U('product_banner_ad/listing'), L('路线主推列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->Checkproduct_banner_adData_Post($id);

	        $product_banner_adMod = M('product_banner_ad');

	        if($product_banner_adMod->create()) {
	        	
                //$rst=$this->Checkproduct_banner_adData_Mod($product_banner_adMod,$id);
                $product_banner_adMod->modify_time=time();

	            $result =   $product_banner_adMod->save();
	            if($result) {
	                $this->success('操作成功！', U('product_banner_ad/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
	        ...
	        $product_banner_adMod = M('product_banner_ad');
		    // 读取数据
		    $data =   $product_banner_adMod->find($id);
		    if($data) {
	...
	{
		$module = $UserMod = M('product_banner_ad');
...
*/
	...
	private function Checkproduct_banner_adData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function Checkproduct_banner_adData_Mod(&$User, $user_id=0){
...
/*路线主推图集*/
    ...
    public function listing_product_banner_adphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_banner_adMod = M('product_banner_adphoto');
            $sql=" id in (".$in.") ";
            $product_banner_adMod->where($sql)->delete();

            $this->success('删除成功', U('product_banner_ad/listing_product_banner_adphoto'));
            exit;
        }
        ...
        $this->ModManager = M('product_banner_adphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('product_banner_adphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('路线主推图集列表');
        $PageMenu = array(
            array( U('product_banner_ad/create_product_banner_adphoto'), L('添加路线主推图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_product_banner_adphoto()
    {
        $module = $UserMod = M('product_banner_adphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_product_banner_adphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加路线主推图集');
            $PageMenu = array(
                array( U('product_banner_ad/listing_product_banner_adphoto'), L('路线主推图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_product_banner_adphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($product_banner_adMod->create()) {
                ...
                $result =   $product_banner_adMod->save();
                if($result) {
                    $this->success('操作成功！', U('product_banner_ad/edit_product_banner_adphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*路线主推行程*/
    ...
    public function listing_product_banner_adcircle()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $product_banner_adMod = M('product_banner_adcircle');
            $sql=" id in (".$in.") ";
            $product_banner_adMod->where($sql)->delete();

            $this->success('删除成功', U('product_banner_ad/listing_product_banner_adcircle'));
            exit;
        }
        ...
        $this->ModManager = M('product_banner_adcircle');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('product_banner_adcircle', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('路线主推行程列表');
        $PageMenu = array(
            array( U('product_banner_ad/create_product_banner_adcircle'), L('添加路线主推行程') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_product_banner_adcircle()
    {
        $module = $UserMod = M('product_banner_adcircle');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_product_banner_adcircle()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加路线主推行程');
            $PageMenu = array(
                array( U('product_banner_ad/listing_product_banner_adcircle'), L('路线主推行程列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_product_banner_adcircle()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($product_banner_adMod->create()) {
                ...
                $result =   $product_banner_adMod->save();
                if($result) {
                    $this->success('操作成功！', U('product_banner_ad/edit_product_banner_adcircle', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
