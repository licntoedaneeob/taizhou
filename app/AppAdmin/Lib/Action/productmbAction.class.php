 ...
 *
 */
class productmbAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $productmbMod = M('productmb');
            $sql=" id in (".$in.") ";
            $productmbMod->where($sql)->delete();

            $this->success('删除成功', U('productmb/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('productmb', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('产品专区列表');
		$PageMenu = array(
			array( U('productmb/create'), L('添加产品专区') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckproductmbData_Post();

            if (false === $productmbMod->create()) {
                $this->error($module->getError());
            }

            if($productmbMod->create()) {

                //$rst=$this->CheckproductmbData_Mod($productmbMod);
                $productmbMod->create_time=time();
                $productmbMod->modify_time=time();

                $result =   $productmbMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
                ...
                $this->error($productmbMod->getError());
            }
            ...
            $PageTitle = L('添加产品专区');
            $PageMenu = array(
                array( U('productmb/listing'), L('产品专区列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckproductmbData_Post($id);

	        $productmbMod = M('productmb');

	        if($productmbMod->create()) {
	        	
                //$rst=$this->CheckproductmbData_Mod($productmbMod,$id);
                $productmbMod->modify_time=time();

	            $result =   $productmbMod->save();
	            if($result) {
	                $this->success('操作成功！', U('productmb/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckproductmbData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckproductmbData_Mod(&$User, $user_id=0){
...
/*产品专区图集*/
    ...
    public function listing_productmbphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $productmbMod = M('productmbphoto');
            $sql=" id in (".$in.") ";
            $productmbMod->where($sql)->delete();

            $this->success('删除成功', U('productmb/listing_productmbphoto'));
            exit;
        }
        ...
        $this->ModManager = M('productmbphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('productmbphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('产品专区图集列表');
        $PageMenu = array(
            array( U('productmb/create_productmbphoto'), L('添加产品专区图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_productmbphoto()
    {
        $module = $UserMod = M('productmbphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_productmbphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加产品专区图集');
            $PageMenu = array(
                array( U('productmb/listing_productmbphoto'), L('产品专区图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_productmbphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($productmbMod->create()) {
                ...
                $result =   $productmbMod->save();
                if($result) {
                    $this->success('操作成功！', U('productmb/edit_productmbphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
