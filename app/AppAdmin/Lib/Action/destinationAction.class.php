 ...
 *
 */
class destinationAction extends TAction
{
...
//    }
	...
	{	
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $destinationMod = M('destination');
            $sql=" id in (".$in.") ";
            $destinationMod->where($sql)->delete();

            $this->success('删除成功', U('destination/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('destination', $sqlWhere, $sqlOrder, '', 'M');
		...
		$PageTitle = L('目的地列表');
		$PageMenu = array(
			array( U('destination/create'), L('添加目的地') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckdestinationData_Post();

            if (false === $destinationMod->create()) {
                $this->error($module->getError());
            }

            if($destinationMod->create()) {

                //$rst=$this->CheckdestinationData_Mod($destinationMod);
                $destinationMod->create_time=time();
                $destinationMod->modify_time=time();

                $result =   $destinationMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
                ...
                $this->error($destinationMod->getError());
            }
            ...
            $PageTitle = L('添加目的地');
            $PageMenu = array(
                array( U('destination/listing'), L('目的地列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckdestinationData_Post($id);

	        $destinationMod = M('destination');

	        if($destinationMod->create()) {
	        	
                //$rst=$this->CheckdestinationData_Mod($destinationMod,$id);
                $destinationMod->modify_time=time();

	            $result =   $destinationMod->save();
	            if($result) {
	                $this->success('操作成功！', U('destination/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckdestinationData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckdestinationData_Mod(&$User, $user_id=0){
...
/*目的地图集*/
    ...
    public function listing_destinationphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $destinationMod = M('destinationphoto');
            $sql=" id in (".$in.") ";
            $destinationMod->where($sql)->delete();

            $this->success('删除成功', U('destination/listing_destinationphoto'));
            exit;
        }
        ...
        $this->ModManager = M('destinationphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('destinationphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('目的地图集列表');
        $PageMenu = array(
            array( U('destination/create_destinationphoto'), L('添加目的地图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_destinationphoto()
    {
        $module = $UserMod = M('destinationphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_destinationphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加目的地图集');
            $PageMenu = array(
                array( U('destination/listing_destinationphoto'), L('目的地图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_destinationphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($destinationMod->create()) {
                ...
                $result =   $destinationMod->save();
                if($result) {
                    $this->success('操作成功！', U('destination/edit_destinationphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
...
/*目的地行程*/
    ...
    public function listing_destinationcircle()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $destinationMod = M('destinationcircle');
            $sql=" id in (".$in.") ";
            $destinationMod->where($sql)->delete();

            $this->success('删除成功', U('destination/listing_destinationcircle'));
            exit;
        }
        ...
        $this->ModManager = M('destinationcircle');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('destinationcircle', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('目的地行程列表');
        $PageMenu = array(
            array( U('destination/create_destinationcircle'), L('添加目的地行程') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_destinationcircle()
    {
        $module = $UserMod = M('destinationcircle');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_destinationcircle()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加目的地行程');
            $PageMenu = array(
                array( U('destination/listing_destinationcircle'), L('目的地行程列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_destinationcircle()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($destinationMod->create()) {
                ...
                $result =   $destinationMod->save();
                if($result) {
                    $this->success('操作成功！', U('destination/edit_destinationcircle', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
