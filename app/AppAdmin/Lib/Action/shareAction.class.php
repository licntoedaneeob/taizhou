 ...
 *
 */
class shareAction extends TAction
{
...
/*达人分享*/
    ...
    public function listing_share()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $this->success('删除成功', U('share/listing_share'));
            exit;
        }
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('share', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('达人分享列表');
        $PageMenu = array(
            array( U('share/create_share'), L('添加达人分享') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_share()
    {
        $module = $UserMod = M('share');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_share()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加达人分享');
            $PageMenu = array(
                array( U('share/listing_share'), L('达人分享列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_share()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($NewsMod->create()) {
                ...
                $result =   $NewsMod->save();
                if($result) {
                    $this->success('操作成功！', U('share/edit_share', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
