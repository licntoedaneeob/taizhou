 ...
 *
 */
class worksmbAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $worksmbMod = M('worksmb');
            $sql=" id in (".$in.") ";
            $worksmbMod->where($sql)->delete();

            $this->success('删除成功', U('worksmb/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('worksmb', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('酷视频列表');
		$PageMenu = array(
			array( U('worksmb/create'), L('添加酷视频') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckworksmbData_Post();

            if (false === $worksmbMod->create()) {
                $this->error($module->getError());
            }

            if($worksmbMod->create()) {

                //$rst=$this->CheckworksmbData_Mod($worksmbMod);
                $worksmbMod->create_time=time();
                $worksmbMod->modify_time=time();

                $result =   $worksmbMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
                ...
                $this->error($worksmbMod->getError());
            }
            ...
            $PageTitle = L('添加酷视频');
            $PageMenu = array(
                array( U('worksmb/listing'), L('酷视频列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckworksmbData_Post($id);

	        $worksmbMod = M('worksmb');

	        if($worksmbMod->create()) {
	        	
                //$rst=$this->CheckworksmbData_Mod($worksmbMod,$id);
                $worksmbMod->modify_time=time();

	            $result =   $worksmbMod->save();
	            if($result) {
	                $this->success('操作成功！', U('worksmb/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckworksmbData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckworksmbData_Mod(&$User, $user_id=0){
...
/*酷视频图集*/
    ...
    public function listing_worksmbphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $worksmbMod = M('worksmbphoto');
            $sql=" id in (".$in.") ";
            $worksmbMod->where($sql)->delete();

            $this->success('删除成功', U('worksmb/listing_worksmbphoto'));
            exit;
        }
        ...
        $this->ModManager = M('worksmbphoto');
        $f_order = $this->REQUEST('_f_order', 'id');
        $fields = $this->ModManager->getDbFields();
        //echo "<pre>";print_r($fields);exit;
        if( in_array($f_order, $fields) ){
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('worksmbphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('酷视频图集列表');
        $PageMenu = array(
            array( U('worksmb/create_worksmbphoto'), L('添加酷视频图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_worksmbphoto()
    {
        $module = $UserMod = M('worksmbphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_worksmbphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加酷视频图集');
            $PageMenu = array(
                array( U('worksmb/listing_worksmbphoto'), L('酷视频图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_worksmbphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($worksmbMod->create()) {
                ...
                $result =   $worksmbMod->save();
                if($result) {
                    $this->success('操作成功！', U('worksmb/edit_worksmbphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
