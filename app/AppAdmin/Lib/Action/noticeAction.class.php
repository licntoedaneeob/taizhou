 ...
 *
 */
class noticeAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $NoticeMod = M('notice');
            $sql=" id in (".$in.") ";
            $NoticeMod->where($sql)->delete();

            $this->success('删除成功', U('notice/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('notice', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('公告列表');
		$PageMenu = array(
			array( U('notice/create'), L('添加公告') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckNoticeData_Post();

            if (false === $NoticeMod->create()) {
                $this->error($module->getError());
            }

            if($NoticeMod->create()) {

                //$rst=$this->CheckNoticeData_Mod($NoticeMod);
                $NoticeMod->create_time=time();

                $result =   $NoticeMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckNoticeData_Post($id);

	        $NoticeMod = M('notice');

	        if($NoticeMod->create()) {
	        	
                //$rst=$this->CheckNoticeData_Mod($NoticeMod,$id);
                $NoticeMod->modify_time=time();

	            $result =   $NoticeMod->save();
	            if($result) {
	                $this->success('操作成功！', U('notice/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckNoticeData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckNoticeData_Mod(&$User, $user_id=0){
