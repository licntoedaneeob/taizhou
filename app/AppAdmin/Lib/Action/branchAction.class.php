 ...
 *
 */
class branchAction extends TAction
{
...
//    }
	...
	public function listing()
	{
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            $branchMod = M('branch');
            $sql=" id in (".$in.") ";
            $branchMod->where($sql)->delete();

            $this->success('删除成功', U('branch/listing'));
            exit;
        }
		...
		///获取列表数据集		
		$rst=$this->GeneralActionForListing('branch', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;

		$PageTitle = L('分支机构列表');
		$PageMenu = array(
			array( U('branch/create'), L('添加分支机构') ),
		);
    ...
    public function create()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckbranchData_Post();

            if (false === $branchMod->create()) {
                $this->error($module->getError());
            }

            if($branchMod->create()) {

                //$rst=$this->CheckbranchData_Mod($branchMod);
                $branchMod->create_time=time();
                $branchMod->modify_time=time();

                $result =   $branchMod->add();
                if($result) {
                    $this->success('操作成功！');
                }else{
                    $this->error('写入错误！');
            ...
            $PageTitle = L('添加分支机构');
            $PageMenu = array(
                array( U('branch/listing'), L('分支机构列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
	...
	public function edit()
	{
		...
		if(isset($_POST['dosubmit'])){
            ...
            //$rst=$this->CheckbranchData_Post($id);

	        $branchMod = M('branch');

	        if($branchMod->create()) {
	        	
                //$rst=$this->CheckbranchData_Mod($branchMod,$id);
                $branchMod->modify_time=time();

	            $result =   $branchMod->save();
	            if($result) {
	                $this->success('操作成功！', U('branch/edit', array('id'=>$id)) );
	            }else{
	                $this->error('写入错误！');
	            }
	        }else{
...
*/
	...
	private function CheckbranchData_Post($user_id=0){
		///检查 $_POST 提交数据

	}
	

	private function CheckbranchData_Mod(&$User, $user_id=0){
...
/*分支机构图集*/
    ...
    public function listing_branchphoto()
    {
        ...
        if(isset($_POST['dosubmit']) && $_POST['dosubmit']=='删除'){
            ...
            //删除自身数据
            $branchMod = M('branchphoto');
            $sql=" id in (".$in.") ";
            $branchMod->where($sql)->delete();

            $this->success('删除成功', U('branch/listing_branchphoto'));
            exit;
        }
        ...
        ///获取列表数据集
        $rst=$this->GeneralActionForListing('branchphoto', $sqlWhere, $sqlOrder, '', 'M');
        //echo "<pre>";print_r($rst);exit;
        ...
        $PageTitle = L('分支机构图集列表');
        $PageMenu = array(
            array( U('branch/create_branchphoto'), L('添加分支机构图集') ),
        );
        $this->setPagething( $PageTitle, $PageMenu, true);
        $this->display();
    }

    public function ajax_change_status_branchphoto()
    {
        $module = $UserMod = M('branchphoto');
        $id 	= intval($_REQUEST['id']);
        //$type 	= isset($_REQUEST['type']) ? trim($_REQUEST['type']) : 'status';
        $status = $type=($type+1)%2;
    ...
    public function create_branchphoto()
    {
        $allclasslist=$this->getAllClassList();
        $this->assign('allclasslist', $allclasslist );
        ...
        if(isset($_POST['dosubmit'])){
            ...
            $PageTitle = L('添加分支机构图集');
            $PageMenu = array(
                array( U('branch/listing_branchphoto'), L('分支机构图集列表') ),
            );
            $this->setPagething( $PageTitle, $PageMenu, true);
            $this->display();
        }
    ...
    public function edit_branchphoto()
    {
        ...
        if(isset($_POST['dosubmit'])){
            ...
            if($branchMod->create()) {
                ...
                $result =   $branchMod->save();
                if($result) {
                    $this->success('操作成功！', U('branch/edit_branchphoto', array('id'=>$id)) );
                }else{
                    $this->error('写入错误！');
                }
            }else{
