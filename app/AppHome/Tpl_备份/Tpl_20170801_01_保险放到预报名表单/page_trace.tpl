<?php
if (!function_exists('lkShowTrackValue')) { 
function lkShowTrackValue($value){
...
<div id="think_page_trace_tab_tit" style="height:30px;padding: 6px 12px 0;border-bottom:1px solid #ececec;border-top:1px solid #ececec;font-size:16px">
    ...
    <span style="color:#000;padding-right:12px;height:30px;line-height: 30px;display:inline-block;margin-right:3px;cursor: pointer;font-weight:700"><?php echo lkShowTrackValue($key) ?></span>
    <?php } ?>
</div>
<div id="think_page_trace_tab_cont" style="overflow:auto;height:212px;padding: 0; line-height: 24px;">
	...
	if(is_array($info)){
		...
		echo '<li style="border-bottom:1px solid #666;font-size:12px;font-family: arial; line-height: 1.2em; padding:0 12px">' . (is_numeric($k) ? '' : $k.' : ') . '<pre style="font-size:12px;font-family: tahoma,​arial,​Verdana,​sans-serif;">'.htmlentities(lkShowTrackValue($val),ENT_COMPAT,'utf-8') .'</pre></li>';
	    }
	}
    ?>
    </ol>
...
<?php /* 隐藏trace写法：bottom:-30px;  显示trace写法：bottom:0px; */ ?>
<div id="think_page_trace_open" style="height:30px;float:right;text-align: right;overflow:hidden;position:fixed;bottom:-30px;right:0;color:#000;line-height:30px;cursor:pointer;"><div style="background:#232323;color:#FFF;padding:0 6px;float:right;line-height:30px;font-size:14px"><?php echo G('beginTime','viewEndTime').'s ';?></div><img width="30" style="" title="ShowPageTrace" src="data:image/png;base64,<?php echo App::logo() ?>"></div>
