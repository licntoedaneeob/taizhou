...
return res;}
function renderTag(tagName,parentView,tmpl,tagCtxs){var render,tag,tags,attr,isElse,parentTag,i,l,itemRet,tagCtx,tagCtxCtx,content,boundTagFn,tagDef,callInit,ret="",boundTagKey=+tagCtxs===tagCtxs&&tagCtxs,linkCtx=parentView.linkCtx||0,ctx=parentView.ctx,parentTmpl=tmpl||parentView.tmpl,parentView_=parentView._;if(tagName._is==="tag"){tag=tagName;tagName=tag.tagName;}
...
tmpl=tmpl||(tag?tag._def:tagDef).template||content;tmpl=""+tmpl===tmpl?parentView.getRsc("templates",tmpl)||$templates(tmpl):tmpl;$extend(tagCtx,{tmpl:tmpl,render:renderContent,index:i,view:parentView,ctx:extendCtx(tagCtx.ctx,ctx)});if(!tag){if(tagDef._ctr){tag=new tagDef._ctr();callInit=!!tag.init;tag.attr=tag.attr||tagDef.attr||undefined;}else{tag={render:tagDef.render};}
...
tag.tagName=tagName;tag.parent=parentTag=ctx&&ctx.tag,tag._is="tag";tag._def=tagDef;}
parentView_.tag=tag;tagCtx.tag=tag;tag.tagCtxs=tagCtxs;if(!tag.flow){tagCtxCtx=tagCtx.ctx=tagCtx.ctx||{};tags=tag.parents=tagCtxCtx.parentTags=ctx&&extendCtx(tagCtxCtx.parentTags,ctx.parentTags)||{};if(parentTag){tags[parentTag.tagName]=parentTag;}
tagCtxCtx.tag=tag;}
tag.rendering={};}
for(i=0;i<l;i++){tagCtx=tag.tagCtx=tagCtxs[i];tag.ctx=tagCtx.ctx;if(!i&&callInit){tag.init(tagCtx,linkCtx,tag.ctx);callInit=undefined;}
if(render=tag.render){itemRet=render.apply(tag,tagCtx.args);}
...
function compileTag(name,tagDef,parentTmpl){var init,tmpl;if(typeof tagDef==="function"){tagDef={depends:tagDef.depends,render:tagDef};}else{if(tmpl=tagDef.template){tagDef.template=""+tmpl===tmpl?($templates[tmpl]||$templates(tmpl)):tmpl;}
if(tagDef.init!==false){init=tagDef._ctr=function(tagCtx){};(init.prototype=tagDef).constructor=init;}}
if(parentTmpl){tagDef._parentTmpl=parentTmpl;}
return tagDef;}
...
return"";}
function error(message){throw new $views.sub.Error(message);}
...
ev.done=true;},flow:true},include:{flow:true},"*":{render:function(value){return value;},flow:true}});function getCharEntity(ch){return charEntities[ch]||(charEntities[ch]="&#"+ch.charCodeAt(0)+";");}
